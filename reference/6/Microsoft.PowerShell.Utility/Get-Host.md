...
宿主对象的 CurrentCulture 属性包含一个 **CurrentCulture** 对象 **。** 此命令使用管道运算符 (|) 将 **CultureInfo** 对象发送到 `Format-List` cmdlet。 该 `Format-List` 命令使用值为 all ( * ) 的 *Property* 参数来显示 **CultureInfo** 对象的所有属性和属性值。
