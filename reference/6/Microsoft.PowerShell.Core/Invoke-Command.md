...
### <span data-ttu-id="1c177-109">会话</span><span class="sxs-lookup"><span data-stu-id="1c177-109">Session</span></span>
...
### <span data-ttu-id="1c177-119">ContainerId</span><span class="sxs-lookup"><span data-stu-id="1c177-119">ContainerId</span></span>
...
<span data-ttu-id="1c177-143">此示例在 `Test.ps1` Server01 计算机上运行脚本。</span><span class="sxs-lookup"><span data-stu-id="1c177-143">This example runs the `Test.ps1` script on the Server01 computer.</span></span>
...
<span data-ttu-id="1c177-157">`Invoke-Command`Cmdlet `Get-Culture` 在 Server02 上运行命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-157">The `Invoke-Command` cmdlet runs the `Get-Culture` command on Server02.</span></span> <span data-ttu-id="1c177-158">**Session** 参数指定保存在变量中的会话 `$s` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-158">The **Session** parameter specifies the session saved in the `$s` variable.</span></span>
...
### <span data-ttu-id="1c177-184">示例7：获取多台计算机上的主机程序版本</span><span class="sxs-lookup"><span data-stu-id="1c177-184">Example 7: Get the version of the host program on multiple computers</span></span>
...
### <span data-ttu-id="1c177-207">示例9：在远程计算机上运行的命令中包括局部变量</span><span class="sxs-lookup"><span data-stu-id="1c177-207">Example 9: Include local variables in a command run on a remote computer</span></span>
...
### <span data-ttu-id="1c177-231">示例12：使用脚本块中的 $args 自动变量</span><span class="sxs-lookup"><span data-stu-id="1c177-231">Example 12: Use the $args automatic variable in a script block</span></span>
...
### <span data-ttu-id="1c177-255">示例15：使用 session 选项</span><span class="sxs-lookup"><span data-stu-id="1c177-255">Example 15: Use a session option</span></span>
...
<span data-ttu-id="1c177-278">`Enable-WSManCredSSP`Cmdlet 可启用从 Server01 本地计算机到 Server02 远程计算机的 CredSSP 委托。</span><span class="sxs-lookup"><span data-stu-id="1c177-278">The `Enable-WSManCredSSP` cmdlet enables CredSSP delegation from the Server01 local computer to the Server02 remote computer.</span></span> <span data-ttu-id="1c177-279">**Role** 参数指定 **客户端** 以在本地计算机上配置 CredSSP 客户端设置。</span><span class="sxs-lookup"><span data-stu-id="1c177-279">The **Role** parameter specifies **Client** to configure the CredSSP client setting on the local computer.</span></span>
...
### <span data-ttu-id="1c177-306">示例21：使用 SSH 作为作业在多台远程计算机上运行脚本文件</span><span class="sxs-lookup"><span data-stu-id="1c177-306">Example 21: Run a script file on multiple remote computers using SSH as a job</span></span>
...
### <span data-ttu-id="1c177-318">-ApplicationName</span><span class="sxs-lookup"><span data-stu-id="1c177-318">-ApplicationName</span></span>
...
<span data-ttu-id="1c177-333">**ArgumentList** 参数中的值可以是实际值（例如1024），也可以是对局部变量（如）的引用 `$max` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-333">The values in the **ArgumentList** parameter can be actual values, such as 1024, or they can be references to local variables, such as `$max`.</span></span>
...
<span data-ttu-id="1c177-361">默认值为 Default。</span><span class="sxs-lookup"><span data-stu-id="1c177-361">The default value is Default.</span></span>
...
<span data-ttu-id="1c177-372">若要获取证书指纹，请 `Get-Item` `Get-ChildItem` 在 PowerShell Cert：驱动器中使用或命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-372">To get a certificate thumbprint, use a `Get-Item` or `Get-ChildItem` command in the PowerShell Cert: drive.</span></span>
...
### <span data-ttu-id="1c177-395">-ConnectionUri</span><span class="sxs-lookup"><span data-stu-id="1c177-395">-ConnectionUri</span></span>
...
<span data-ttu-id="1c177-401">URI 的 **Transport** 段的有效值为 HTTP 和 HTTPS。</span><span class="sxs-lookup"><span data-stu-id="1c177-401">Valid values for the **Transport** segment of the URI are HTTP and HTTPS.</span></span> <span data-ttu-id="1c177-402">如果使用传输段指定连接 URI，但未指定端口，则将使用标准端口：80用于 HTTP，443用于 HTTPS。</span><span class="sxs-lookup"><span data-stu-id="1c177-402">If you specify a connection URI with a Transport segment, but don't specify a port, the session is created with the standards ports: 80 for HTTP and 443 for HTTPS.</span></span> <span data-ttu-id="1c177-403">若要使用 PowerShell 远程处理的默认端口，请为 HTTP 指定端口5985，为 HTTPS 指定5986。</span><span class="sxs-lookup"><span data-stu-id="1c177-403">To use the default ports for PowerShell remoting, specify port 5985 for HTTP or 5986 for HTTPS.</span></span>

<span data-ttu-id="1c177-404">如果目标计算机将连接重定向到不同的 URI，则 PowerShell 会阻止重定向，除非你在命令中使用 **AllowRedirection** 参数。</span><span class="sxs-lookup"><span data-stu-id="1c177-404">If the destination computer redirects the connection to a different URI, PowerShell prevents the redirection unless you use the **AllowRedirection** parameter in the command.</span></span>
...
### <span data-ttu-id="1c177-405">-ContainerId</span><span class="sxs-lookup"><span data-stu-id="1c177-405">-ContainerId</span></span>

<span data-ttu-id="1c177-406">指定容器 Id 的数组。</span><span class="sxs-lookup"><span data-stu-id="1c177-406">Specifies an array of container IDs.</span></span>
...
### <span data-ttu-id="1c177-407">-Credential</span><span class="sxs-lookup"><span data-stu-id="1c177-407">-Credential</span></span>

<span data-ttu-id="1c177-408">指定有权执行此操作的用户帐户。</span><span class="sxs-lookup"><span data-stu-id="1c177-408">Specifies a user account that has permission to perform this action.</span></span> <span data-ttu-id="1c177-409">默认为当前用户。</span><span class="sxs-lookup"><span data-stu-id="1c177-409">The default is the current user.</span></span>

<span data-ttu-id="1c177-410">键入用户名（如 **User01** 或 **Domain01\User01** ），或输入 cmdlet 生成的 **PSCredential** 对象 `Get-Credential` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-410">Type a user name, such as **User01** or **Domain01\User01** , or enter a **PSCredential** object generated by the `Get-Credential` cmdlet.</span></span> <span data-ttu-id="1c177-411">如果键入用户名，系统会提示输入密码。</span><span class="sxs-lookup"><span data-stu-id="1c177-411">If you type a user name, you're prompted to enter the password.</span></span>

<span data-ttu-id="1c177-412">凭据存储在 [PSCredential](/dotnet/api/system.management.automation.pscredential) 对象中，密码存储为 [SecureString](/dotnet/api/system.security.securestring)。</span><span class="sxs-lookup"><span data-stu-id="1c177-412">Credentials are stored in a [PSCredential](/dotnet/api/system.management.automation.pscredential) object and the password is stored as a [SecureString](/dotnet/api/system.security.securestring).</span></span>

> [!NOTE]
> <span data-ttu-id="1c177-413">有关 **SecureString** 数据保护的详细信息，请参阅 [SecureString 的安全级别？](/dotnet/api/system.security.securestring#how-secure-is-securestring)。</span><span class="sxs-lookup"><span data-stu-id="1c177-413">For more information about **SecureString** data protection, see [How secure is SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span></span>
...
### <span data-ttu-id="1c177-414">-EnableNetworkAccess</span><span class="sxs-lookup"><span data-stu-id="1c177-414">-EnableNetworkAccess</span></span>

<span data-ttu-id="1c177-415">指示此 cmdlet 将交互式安全令牌添加到环回会话。</span><span class="sxs-lookup"><span data-stu-id="1c177-415">Indicates that this cmdlet adds an interactive security token to loopback sessions.</span></span> <span data-ttu-id="1c177-416">通过交互式令牌，你可以在环回会话中运行用于获取其他计算机中的数据的命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-416">The interactive token lets you run commands in the loopback session that get data from other computers.</span></span> <span data-ttu-id="1c177-417">例如，你可以在该会话中运行用于将 XML 文件从远程计算机复制到本地计算机的命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-417">For example, you can run a command in the session that copies XML files from a remote computer to the local computer.</span></span>

<span data-ttu-id="1c177-418">环回会话是在同一计算机上开始并终止的 **PSSession** 。</span><span class="sxs-lookup"><span data-stu-id="1c177-418">A loopback session is a **PSSession** that originates and ends on the same computer.</span></span> <span data-ttu-id="1c177-419">若要创建环回会话，请省略 **ComputerName** 参数，或将其值设置为点 (`.`) 、localhost 或本地计算机的名称。</span><span class="sxs-lookup"><span data-stu-id="1c177-419">To create a loopback session, omit the **ComputerName** parameter or set its value to dot (`.`), localhost, or the name of the local computer.</span></span>

<span data-ttu-id="1c177-420">默认情况下，使用网络令牌创建环回会话，该令牌可能不提供足够的权限来对远程计算机进行身份验证。</span><span class="sxs-lookup"><span data-stu-id="1c177-420">By default, loopback sessions are created using a network token, which might not provide sufficient permission to authenticate to remote computers.</span></span>

<span data-ttu-id="1c177-421">**EnableNetworkAccess** 参数仅在环回会话中有效。</span><span class="sxs-lookup"><span data-stu-id="1c177-421">The **EnableNetworkAccess** parameter is effective only in loopback sessions.</span></span> <span data-ttu-id="1c177-422">如果在远程计算机上创建会话时使用 **EnableNetworkAccess** ，则该命令将成功，但会忽略此参数。</span><span class="sxs-lookup"><span data-stu-id="1c177-422">If you use **EnableNetworkAccess** when you create a session on a remote computer, the command succeeds, but the parameter is ignored.</span></span>

<span data-ttu-id="1c177-423">你可以使用 **Authentication** 参数的 **CredSSP** 值（它将会话凭据委派给其他计算机）允许在环回会话中进行远程访问。</span><span class="sxs-lookup"><span data-stu-id="1c177-423">You can allow remote access in a loopback session using the **CredSSP** value of the **Authentication** parameter, which delegates the session credentials to other computers.</span></span>

<span data-ttu-id="1c177-424">若要防止计算机受到恶意访问，具有交互式令牌（使用 **EnableNetworkAccess** 创建的令牌）的断开连接环回会话只能从创建会话的计算机重新连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-424">To protect the computer from malicious access, disconnected loopback sessions that have interactive tokens, which are those created using **EnableNetworkAccess** , can be reconnected only from the computer on which the session was created.</span></span> <span data-ttu-id="1c177-425">断开连接的使用 CredSSP 身份验证的会话可通过其他计算机重新连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-425">Disconnected sessions that use CredSSP authentication can be reconnected from other computers.</span></span> <span data-ttu-id="1c177-426">有关详细信息，请参阅 `Disconnect-PSSession`。</span><span class="sxs-lookup"><span data-stu-id="1c177-426">For more information, see `Disconnect-PSSession`.</span></span>

<span data-ttu-id="1c177-427">此参数是在 PowerShell 3.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-427">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="1c177-428">-FilePath</span><span class="sxs-lookup"><span data-stu-id="1c177-428">-FilePath</span></span>

<span data-ttu-id="1c177-429">指定此 cmdlet 在一台或多台远程计算机上运行的本地脚本。</span><span class="sxs-lookup"><span data-stu-id="1c177-429">Specifies a local script that this cmdlet runs on one or more remote computers.</span></span> <span data-ttu-id="1c177-430">输入脚本的路径和文件名，或通过管道将脚本路径传递给 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-430">Enter the path and filename of the script, or pipe a script path to `Invoke-Command`.</span></span> <span data-ttu-id="1c177-431">脚本必须位于本地计算机或本地计算机可以访问的目录中。</span><span class="sxs-lookup"><span data-stu-id="1c177-431">The script must exist on the local computer or in a directory that the local computer can access.</span></span> <span data-ttu-id="1c177-432">使用 **ArgumentList** 指定脚本中参数的值。</span><span class="sxs-lookup"><span data-stu-id="1c177-432">Use **ArgumentList** to specify the values of parameters in the script.</span></span>

<span data-ttu-id="1c177-433">当你使用此参数时，PowerShell 会将指定脚本文件的内容转换为脚本块，将脚本块传输到远程计算机，然后在远程计算机上运行它。</span><span class="sxs-lookup"><span data-stu-id="1c177-433">When you use this parameter, PowerShell converts the contents of the specified script file to a script block, transmits the script block to the remote computer, and runs it on the remote computer.</span></span>
...
### <span data-ttu-id="1c177-434">-HideComputerName</span><span class="sxs-lookup"><span data-stu-id="1c177-434">-HideComputerName</span></span>

<span data-ttu-id="1c177-435">指示此 cmdlet 省略输出显示中每个对象的计算机名称。</span><span class="sxs-lookup"><span data-stu-id="1c177-435">Indicates that this cmdlet omits the computer name of each object from the output display.</span></span> <span data-ttu-id="1c177-436">默认情况下，生成该对象的计算机名称会出现在显示内容中。</span><span class="sxs-lookup"><span data-stu-id="1c177-436">By default, the name of the computer that generated the object appears in the display.</span></span>

<span data-ttu-id="1c177-437">此参数仅影响输出显示。</span><span class="sxs-lookup"><span data-stu-id="1c177-437">This parameter affects only the output display.</span></span> <span data-ttu-id="1c177-438">它不会更改对象。</span><span class="sxs-lookup"><span data-stu-id="1c177-438">It doesn't change the object.</span></span>
...
### <span data-ttu-id="1c177-439">-HostName</span><span class="sxs-lookup"><span data-stu-id="1c177-439">-HostName</span></span>

<span data-ttu-id="1c177-440">指定安全外壳 (基于 SSH) 连接的计算机名称的数组。</span><span class="sxs-lookup"><span data-stu-id="1c177-440">Specifies an array of computer names for a Secure Shell (SSH) based connection.</span></span> <span data-ttu-id="1c177-441">这与 **ComputerName** 参数类似，不同之处在于使用 SSH 而不是 Windows WinRM 建立与远程计算机的连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-441">This is similar to the **ComputerName** parameter except that the connection to the remote computer is made using SSH rather than Windows WinRM.</span></span>

<span data-ttu-id="1c177-442">此参数是在 PowerShell 6.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-442">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="1c177-443">-InDisconnectedSession</span><span class="sxs-lookup"><span data-stu-id="1c177-443">-InDisconnectedSession</span></span>

<span data-ttu-id="1c177-444">指示此 cmdlet 在断开连接的会话中运行命令或脚本。</span><span class="sxs-lookup"><span data-stu-id="1c177-444">Indicates that this cmdlet runs a command or script in a disconnected session.</span></span>

<span data-ttu-id="1c177-445">使用 **InDisconnectedSession** 参数时，将 `Invoke-Command` 在每台远程计算机上创建一个持久会话，启动由 **ScriptBlock** 或 **FilePath** 参数指定的命令，然后断开与该会话的连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-445">When you use the **InDisconnectedSession** parameter, `Invoke-Command` creates a persistent session on each remote computer, starts the command specified by the **ScriptBlock** or **FilePath** parameter, and then disconnects from the session.</span></span> <span data-ttu-id="1c177-446">这些命令将继续在断开连接的会话中运行。</span><span class="sxs-lookup"><span data-stu-id="1c177-446">The commands continue to run in the disconnected sessions.</span></span> <span data-ttu-id="1c177-447">**InDisconnectedSession** 使你能够在不保持与远程会话的连接的情况下运行命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-447">**InDisconnectedSession** enables you to run commands without maintaining a connection to the remote sessions.</span></span> <span data-ttu-id="1c177-448">而且，因为在返回任何结果之前会话已断开连接， **InDisconnectedSession** 确保所有命令结果都返回到重新连接的会话，而不是在会话之间拆分。</span><span class="sxs-lookup"><span data-stu-id="1c177-448">And, because the session is disconnected before any results are returned, **InDisconnectedSession** makes sure that all command results are returned to the reconnected session, instead of being split between sessions.</span></span>

<span data-ttu-id="1c177-449">不能将 **InDisconnectedSession** 与 **Session** 参数或 **AsJob** 参数一起使用。</span><span class="sxs-lookup"><span data-stu-id="1c177-449">You can't use **InDisconnectedSession** with the **Session** parameter or the **AsJob** parameter.</span></span>

<span data-ttu-id="1c177-450">使用 **InDisconnectedSession** 的命令返回表示已断开连接的会话的 **PSSession** 对象。</span><span class="sxs-lookup"><span data-stu-id="1c177-450">Commands that use **InDisconnectedSession** return a **PSSession** object that represents the disconnected session.</span></span> <span data-ttu-id="1c177-451">它们不会返回命令输出。</span><span class="sxs-lookup"><span data-stu-id="1c177-451">They don't return the command output.</span></span> <span data-ttu-id="1c177-452">若要连接到断开连接的会话，请使用 `Connect-PSSession` 或 `Receive-PSSession` cmdlet。</span><span class="sxs-lookup"><span data-stu-id="1c177-452">To connect to the disconnected session, use the `Connect-PSSession` or `Receive-PSSession` cmdlets.</span></span> <span data-ttu-id="1c177-453">若要获取在会话中运行的命令的结果，请使用 `Receive-PSSession` cmdlet。</span><span class="sxs-lookup"><span data-stu-id="1c177-453">To get the results of commands that ran in the session, use the `Receive-PSSession` cmdlet.</span></span> <span data-ttu-id="1c177-454">若要运行在断开连接的会话中生成输出的命令，请将 **OutputBufferingMode** session 选项的值设置为 **Drop** 。</span><span class="sxs-lookup"><span data-stu-id="1c177-454">To run commands that generate output in a disconnected session, set the value of the **OutputBufferingMode** session option to **Drop** .</span></span> <span data-ttu-id="1c177-455">如果打算连接到断开连接的会话，请在会话中设置空闲超时，以便在删除会话之前可以提供足够的时间进行连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-455">If you intend to connect to the disconnected session, set the idle time-out in the session so that it provides sufficient time for you to connect before deleting the session.</span></span>

<span data-ttu-id="1c177-456">可以在 **SessionOption** 参数或 `$PSSessionOption` 首选项变量中设置输出缓冲模式和空闲超时。</span><span class="sxs-lookup"><span data-stu-id="1c177-456">You can set the output buffering mode and idle time-out in the **SessionOption** parameter or in the `$PSSessionOption` preference variable.</span></span> <span data-ttu-id="1c177-457">有关会话选项的详细信息，请参阅 `New-PSSessionOption` 和 [about_Preference_Variables](./about/about_preference_variables.md)。</span><span class="sxs-lookup"><span data-stu-id="1c177-457">For more information about session options, see `New-PSSessionOption` and [about_Preference_Variables](./about/about_preference_variables.md).</span></span>

<span data-ttu-id="1c177-458">有关断开连接会话的功能的详细信息，请参阅 [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md)。</span><span class="sxs-lookup"><span data-stu-id="1c177-458">For more information about the Disconnected Sessions feature, see [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span></span>

<span data-ttu-id="1c177-459">此参数是在 PowerShell 3.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-459">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="1c177-460">-InputObject</span><span class="sxs-lookup"><span data-stu-id="1c177-460">-InputObject</span></span>

<span data-ttu-id="1c177-461">指定命令的输入。</span><span class="sxs-lookup"><span data-stu-id="1c177-461">Specifies input to the command.</span></span> <span data-ttu-id="1c177-462">输入一个包含对象的变量，或键入可获取对象的命令或表达式。</span><span class="sxs-lookup"><span data-stu-id="1c177-462">Enter a variable that contains the objects or type a command or expression that gets the objects.</span></span>

<span data-ttu-id="1c177-463">使用 **InputObject** 参数时，请使用 `$Input` **ScriptBlock** 参数的值中的自动变量来表示输入对象。</span><span class="sxs-lookup"><span data-stu-id="1c177-463">When using the **InputObject** parameter, use the `$Input` automatic variable in the value of the **ScriptBlock** parameter to represent the input objects.</span></span>
...
### <span data-ttu-id="1c177-464">-JobName</span><span class="sxs-lookup"><span data-stu-id="1c177-464">-JobName</span></span>

<span data-ttu-id="1c177-465">为后台作业指定友好名称。</span><span class="sxs-lookup"><span data-stu-id="1c177-465">Specifies a friendly name for the background job.</span></span> <span data-ttu-id="1c177-466">默认情况下，作业名为 `Job<n>` ，其中 `<n>` 是一个序号。</span><span class="sxs-lookup"><span data-stu-id="1c177-466">By default, jobs are named `Job<n>`, where `<n>` is an ordinal number.</span></span>

<span data-ttu-id="1c177-467">如果你在命令中使用 **JobName** 参数，则该命令将作为作业运行，并 `Invoke-Command` 返回一个作业对象，即使未在命令中包含 **AsJob** 也是如此。</span><span class="sxs-lookup"><span data-stu-id="1c177-467">If you use the **JobName** parameter in a command, the command is run as a job, and `Invoke-Command` returns a job object, even if you don't include **AsJob** in the command.</span></span>

<span data-ttu-id="1c177-468">有关 PowerShell 后台作业的详细信息，请参阅 [about_Jobs](./About/about_Jobs.md)。</span><span class="sxs-lookup"><span data-stu-id="1c177-468">For more information about PowerShell background jobs, see [about_Jobs](./About/about_Jobs.md).</span></span>
...
### <span data-ttu-id="1c177-469">-KeyFilePath</span><span class="sxs-lookup"><span data-stu-id="1c177-469">-KeyFilePath</span></span>

<span data-ttu-id="1c177-470">指定安全外壳 (SSH) 用于对远程计算机上的用户进行身份验证的密钥文件路径。</span><span class="sxs-lookup"><span data-stu-id="1c177-470">Specifies a key file path used by Secure Shell (SSH) to authenticate a user on a remote computer.</span></span>

<span data-ttu-id="1c177-471">SSH 允许通过私有密钥和公钥执行用户身份验证，作为基本密码身份验证的替代方法。</span><span class="sxs-lookup"><span data-stu-id="1c177-471">SSH allows user authentication to be performed via private and public keys as an alternative to basic password authentication.</span></span> <span data-ttu-id="1c177-472">如果将远程计算机配置为进行密钥身份验证，则可以使用此参数来提供用于标识用户的密钥。</span><span class="sxs-lookup"><span data-stu-id="1c177-472">If the remote computer is configured for key authentication, then this parameter can be used to provide the key that identifies the user.</span></span>

<span data-ttu-id="1c177-473">此参数是在 PowerShell 6.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-473">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="1c177-474">-NoNewScope</span><span class="sxs-lookup"><span data-stu-id="1c177-474">-NoNewScope</span></span>

<span data-ttu-id="1c177-475">指示此 cmdlet 在当前作用域中运行指定的命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-475">Indicates that this cmdlet runs the specified command in the current scope.</span></span> <span data-ttu-id="1c177-476">默认情况下， `Invoke-Command` 在其自己的作用域中运行命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-476">By default, `Invoke-Command` runs commands in their own scope.</span></span>

<span data-ttu-id="1c177-477">此参数仅在当前会话中运行的命令（即同时省略 **ComputerName** 和 **Session** 参数的命令）中有效。</span><span class="sxs-lookup"><span data-stu-id="1c177-477">This parameter is valid only in commands that are run in the current session, that is, commands that omit both the **ComputerName** and **Session** parameters.</span></span>

<span data-ttu-id="1c177-478">此参数是在 PowerShell 3.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-478">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="1c177-479">-Port</span><span class="sxs-lookup"><span data-stu-id="1c177-479">-Port</span></span>

<span data-ttu-id="1c177-480">指定远程计算机上用于此命令的网络端口。</span><span class="sxs-lookup"><span data-stu-id="1c177-480">Specifies the network port on the remote computer that is used for this command.</span></span> <span data-ttu-id="1c177-481">若要连接到一台远程计算机，则必须在该连接所用的端口上侦听远程计算机。</span><span class="sxs-lookup"><span data-stu-id="1c177-481">To connect to a remote computer, the remote computer must be listening on the port that the connection uses.</span></span> <span data-ttu-id="1c177-482">对于 HTTP) ，默认端口为 5985 (WinRM 端口5986， (HTTPS 的 WinRM 端口) 。</span><span class="sxs-lookup"><span data-stu-id="1c177-482">The default ports are 5985 (WinRM port for HTTP) and 5986 (WinRM port for HTTPS).</span></span>

<span data-ttu-id="1c177-483">使用备用端口之前，请在远程计算机上配置 WinRM 侦听器，以便侦听该端口。</span><span class="sxs-lookup"><span data-stu-id="1c177-483">Before using an alternate port, configure the WinRM listener on the remote computer to listen at that port.</span></span> <span data-ttu-id="1c177-484">若要配置侦听器，请在 PowerShell 提示符下键入以下两个命令：</span><span class="sxs-lookup"><span data-stu-id="1c177-484">To configure the listener, type the following two commands at the PowerShell prompt:</span></span>

`Remove-Item -Path WSMan:\Localhost\listener\listener* -Recurse`

`New-Item -Path WSMan:\Localhost\listener -Transport http -Address * -Port \<port-number\>`

<span data-ttu-id="1c177-485">不要使用 **Port** 参数，除非必须这样做。</span><span class="sxs-lookup"><span data-stu-id="1c177-485">Don't use the **Port** parameter unless you must.</span></span> <span data-ttu-id="1c177-486">在命令中设置的端口适用于运行该命令的所有计算机或会话。</span><span class="sxs-lookup"><span data-stu-id="1c177-486">The port that is set in the command applies to all computers or sessions on which the command runs.</span></span> <span data-ttu-id="1c177-487">备用端口设置可能会阻止在所有计算机上运行该命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-487">An alternate port setting might prevent the command from running on all computers.</span></span>
...
### <span data-ttu-id="1c177-488">-RemoteDebug</span><span class="sxs-lookup"><span data-stu-id="1c177-488">-RemoteDebug</span></span>

<span data-ttu-id="1c177-489">用于在远程 PowerShell 会话中以调试模式运行已调用的命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-489">Used to run the invoked command in debug mode in the remote PowerShell session.</span></span>
...
### <span data-ttu-id="1c177-490">-RunAsAdministrator</span><span class="sxs-lookup"><span data-stu-id="1c177-490">-RunAsAdministrator</span></span>

<span data-ttu-id="1c177-491">指示此 cmdlet 以管理员身份调用命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-491">Indicates that this cmdlet invokes a command as an Administrator.</span></span>
...
### <span data-ttu-id="1c177-492">-ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="1c177-492">-ScriptBlock</span></span>

<span data-ttu-id="1c177-493">指定要运行的命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-493">Specifies the commands to run.</span></span> <span data-ttu-id="1c177-494">将命令括在大括号中 `{ }` ，以创建脚本块。</span><span class="sxs-lookup"><span data-stu-id="1c177-494">Enclose the commands in curly braces `{ }` to create a script block.</span></span>
<span data-ttu-id="1c177-495">此参数是必需的。</span><span class="sxs-lookup"><span data-stu-id="1c177-495">This parameter is required.</span></span>

<span data-ttu-id="1c177-496">默认情况下，将在远程计算机上评估命令中的所有变量。</span><span class="sxs-lookup"><span data-stu-id="1c177-496">By default, any variables in the command are evaluated on the remote computer.</span></span> <span data-ttu-id="1c177-497">若要在命令中包括局部变量，请使用 **ArgumentList** 。</span><span class="sxs-lookup"><span data-stu-id="1c177-497">To include local variables in the command, use **ArgumentList** .</span></span>
...
### <span data-ttu-id="1c177-498">-Session</span><span class="sxs-lookup"><span data-stu-id="1c177-498">-Session</span></span>

<span data-ttu-id="1c177-499">指定此 cmdlet 在其中运行命令的会话的数组。</span><span class="sxs-lookup"><span data-stu-id="1c177-499">Specifies an array of sessions in which this cmdlet runs the command.</span></span> <span data-ttu-id="1c177-500">输入包含 **pssession** 对象的变量或创建或获取 **pssession** 对象的命令，例如 `New-PSSession` 或 `Get-PSSession` 命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-500">Enter a variable that contains **PSSession** objects or a command that creates or gets the **PSSession** objects, such as a `New-PSSession` or `Get-PSSession` command.</span></span>

<span data-ttu-id="1c177-501">创建 **PSSession** 时，PowerShell 会建立与远程计算机的持久连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-501">When you create a **PSSession** , PowerShell establishes a persistent connection to the remote computer.</span></span> <span data-ttu-id="1c177-502">使用 **PSSession** 运行共享数据的一系列相关命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-502">Use a **PSSession** to run a series of related commands that share data.</span></span> <span data-ttu-id="1c177-503">若要运行单个命令或一系列不相关的命令，请使用 **ComputerName** 参数。</span><span class="sxs-lookup"><span data-stu-id="1c177-503">To run a single command or a series of unrelated commands, use the **ComputerName** parameter.</span></span> <span data-ttu-id="1c177-504">有关详细信息，请参阅 [about_PSSessions](./About/about_PSSessions.md)。</span><span class="sxs-lookup"><span data-stu-id="1c177-504">For more information, see [about_PSSessions](./About/about_PSSessions.md).</span></span>
...
### <span data-ttu-id="1c177-505">-SessionName</span><span class="sxs-lookup"><span data-stu-id="1c177-505">-SessionName</span></span>

<span data-ttu-id="1c177-506">为断开连接的会话指定一个友好名称。</span><span class="sxs-lookup"><span data-stu-id="1c177-506">Specifies a friendly name for a disconnected session.</span></span> <span data-ttu-id="1c177-507">可以在后续命令中使用此名称来引用会话，例如 `Get-PSSession` 命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-507">You can use the name to refer to the session in subsequent commands, such as a `Get-PSSession` command.</span></span> <span data-ttu-id="1c177-508">此参数只有在与 **InDisconnectedSession** 参数一起使用时才有效。</span><span class="sxs-lookup"><span data-stu-id="1c177-508">This parameter is valid only with the **InDisconnectedSession** parameter.</span></span>

<span data-ttu-id="1c177-509">此参数是在 PowerShell 3.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-509">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="1c177-510">-SessionOption</span><span class="sxs-lookup"><span data-stu-id="1c177-510">-SessionOption</span></span>

<span data-ttu-id="1c177-511">为该会话指定高级选项。</span><span class="sxs-lookup"><span data-stu-id="1c177-511">Specifies advanced options for the session.</span></span> <span data-ttu-id="1c177-512">输入 **SessionOption** 对象（例如使用 cmdlet 创建的对象） `New-PSSessionOption` ，或输入一个哈希表，其中的键是会话选项名称，而值是会话选项的值。</span><span class="sxs-lookup"><span data-stu-id="1c177-512">Enter a **SessionOption** object, such as one that you create using the `New-PSSessionOption` cmdlet, or a hash table in which the keys are session option names and the values are session option values.</span></span>

<span data-ttu-id="1c177-513">选项的默认值取决于 `$PSSessionOption` 首选项变量的值（如果已设置）。</span><span class="sxs-lookup"><span data-stu-id="1c177-513">The default values for the options are determined by the value of the `$PSSessionOption` preference variable, if it's set.</span></span> <span data-ttu-id="1c177-514">否则，通过在会话配置中设置的选项创建默认值。</span><span class="sxs-lookup"><span data-stu-id="1c177-514">Otherwise, the default values are established by options set in the session configuration.</span></span>

<span data-ttu-id="1c177-515">会话选项值优先于在 `$PSSessionOption` 首选项变量和会话配置中设置的会话的默认值。</span><span class="sxs-lookup"><span data-stu-id="1c177-515">The session option values take precedence over default values for sessions set in the `$PSSessionOption` preference variable and in the session configuration.</span></span> <span data-ttu-id="1c177-516">但是，它们不优先于在会话配置中设置的最大值、配额或限制。</span><span class="sxs-lookup"><span data-stu-id="1c177-516">However, they don't take precedence over maximum values, quotas, or limits set in the session configuration.</span></span>

<span data-ttu-id="1c177-517">有关包括默认值的会话选项的说明，请参阅 `New-PSSessionOption` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-517">For a description of the session options that includes the default values, see `New-PSSessionOption`.</span></span> <span data-ttu-id="1c177-518">有关 `$PSSessionOption` 首选项变量的信息，请参阅 [about_Preference_Variables](About/about_Preference_Variables.md)。</span><span class="sxs-lookup"><span data-stu-id="1c177-518">For information about the `$PSSessionOption` preference variable, see [about_Preference_Variables](About/about_Preference_Variables.md).</span></span>
<span data-ttu-id="1c177-519">有关会话配置的详细信息，请参阅 [about_Session_Configurations](About/about_Session_Configurations.md)。</span><span class="sxs-lookup"><span data-stu-id="1c177-519">For more information about session configurations, see [about_Session_Configurations](About/about_Session_Configurations.md).</span></span>
...
### <span data-ttu-id="1c177-520">-SSHConnection</span><span class="sxs-lookup"><span data-stu-id="1c177-520">-SSHConnection</span></span>

<span data-ttu-id="1c177-521">此参数采用哈希表数组，其中每个哈希表都包含一个或多个建立安全外壳 (SSH) 连接所需的连接参数。</span><span class="sxs-lookup"><span data-stu-id="1c177-521">This parameter takes an array of hash tables where each hash table contains one or more connection parameters needed to establish a Secure Shell (SSH) connection.</span></span> <span data-ttu-id="1c177-522">**SSHConnection** 参数适用于创建多个会话，其中每个会话需要不同的连接信息。</span><span class="sxs-lookup"><span data-stu-id="1c177-522">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>

<span data-ttu-id="1c177-523">哈希表具有以下成员：</span><span class="sxs-lookup"><span data-stu-id="1c177-523">The hashtable has the following members:</span></span>

- <span data-ttu-id="1c177-524">**ComputerName** (或 **主机名** ) </span><span class="sxs-lookup"><span data-stu-id="1c177-524">**ComputerName** (or **HostName** )</span></span>
- <span data-ttu-id="1c177-525">**端口**</span><span class="sxs-lookup"><span data-stu-id="1c177-525">**Port**</span></span>
- <span data-ttu-id="1c177-526">**UserName**</span><span class="sxs-lookup"><span data-stu-id="1c177-526">**UserName**</span></span>
- <span data-ttu-id="1c177-527">**KeyFilePath** (或 **IdentityFilePath** ) </span><span class="sxs-lookup"><span data-stu-id="1c177-527">**KeyFilePath** (or **IdentityFilePath** )</span></span>

<span data-ttu-id="1c177-528">**ComputerName** (或 **HostName** ) 是所需的唯一键值对。</span><span class="sxs-lookup"><span data-stu-id="1c177-528">**ComputerName** (or **HostName** ) is the only key-value pair that is required.</span></span>

<span data-ttu-id="1c177-529">此参数是在 PowerShell 6.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-529">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="1c177-530">-SSHTransport</span><span class="sxs-lookup"><span data-stu-id="1c177-530">-SSHTransport</span></span>

<span data-ttu-id="1c177-531">指示使用安全外壳 (SSH) 建立远程连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-531">Indicates that the remote connection is established using Secure Shell (SSH).</span></span>

<span data-ttu-id="1c177-532">默认情况下，PowerShell 使用 Windows WinRM 连接到远程计算机。</span><span class="sxs-lookup"><span data-stu-id="1c177-532">By default PowerShell uses Windows WinRM to connect to a remote computer.</span></span> <span data-ttu-id="1c177-533">此开关强制 PowerShell 使用 **HostName** 参数建立基于 SSH 的远程连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-533">This switch forces PowerShell to use the **HostName** parameter for establishing an SSH based remote connection.</span></span>

<span data-ttu-id="1c177-534">此参数是在 PowerShell 6.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-534">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="1c177-535">-ThrottleLimit</span><span class="sxs-lookup"><span data-stu-id="1c177-535">-ThrottleLimit</span></span>

<span data-ttu-id="1c177-536">指定为运行此命令可建立的并发连接的最大数目。</span><span class="sxs-lookup"><span data-stu-id="1c177-536">Specifies the maximum number of concurrent connections that can be established to run this command.</span></span>
<span data-ttu-id="1c177-537">如果省略此参数或输入 0 值，则使用默认值 32。</span><span class="sxs-lookup"><span data-stu-id="1c177-537">If you omit this parameter or enter a value of 0, the default value, 32, is used.</span></span>

<span data-ttu-id="1c177-538">节流限制仅适用于当前命令，而不适用于会话或计算机。</span><span class="sxs-lookup"><span data-stu-id="1c177-538">The throttle limit applies only to the current command, not to the session or to the computer.</span></span>
...
### <span data-ttu-id="1c177-539">-UserName</span><span class="sxs-lookup"><span data-stu-id="1c177-539">-UserName</span></span>

<span data-ttu-id="1c177-540">指定用于在远程计算机上运行命令的帐户的用户名。</span><span class="sxs-lookup"><span data-stu-id="1c177-540">Specifies the username for the account used to run a command on the remote computer.</span></span> <span data-ttu-id="1c177-541">用户身份验证方法取决于如何在远程计算机上配置 (SSH) 安全外壳。</span><span class="sxs-lookup"><span data-stu-id="1c177-541">The user authentication method depends on how Secure Shell (SSH) is configured on the remote computer.</span></span>

<span data-ttu-id="1c177-542">如果 SSH 配置了基本密码身份验证，则系统将提示你输入用户密码。</span><span class="sxs-lookup"><span data-stu-id="1c177-542">If SSH is configured for basic password authentication, then you'll be prompted for the user password.</span></span>

<span data-ttu-id="1c177-543">如果 SSH 配置为基于密钥的用户身份验证，则可以通过 **KeyFilePath** 参数提供密钥文件路径，并且不会出现密码提示。</span><span class="sxs-lookup"><span data-stu-id="1c177-543">If SSH is configured for key-based user authentication then a key file path can be provided via the **KeyFilePath** parameter and no password prompt occurs.</span></span> <span data-ttu-id="1c177-544">如果客户端用户密钥文件位于 SSH 已知位置，则基于密钥的身份验证不需要 **KeyFilePath** 参数，并且根据用户名自动进行用户身份验证。</span><span class="sxs-lookup"><span data-stu-id="1c177-544">If the client user key file is located in an SSH known location, then the **KeyFilePath** parameter isn't needed for key-based authentication, and user authentication occurs automatically based on the username.</span></span> <span data-ttu-id="1c177-545">有关详细信息，请参阅平台的 SSH 文档，了解基于密钥的用户身份验证。</span><span class="sxs-lookup"><span data-stu-id="1c177-545">For more information, see your platform's SSH documentation about key-based user authentication.</span></span>

<span data-ttu-id="1c177-546">这不是必需的参数。</span><span class="sxs-lookup"><span data-stu-id="1c177-546">This isn't a required parameter.</span></span> <span data-ttu-id="1c177-547">如果未指定 **UserName** 参数，则将使用当前登录的用户名进行连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-547">If the **UserName** parameter isn't specified, then the current logged on username is used for the connection.</span></span>

<span data-ttu-id="1c177-548">此参数是在 PowerShell 6.0 中引入的。</span><span class="sxs-lookup"><span data-stu-id="1c177-548">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="1c177-549">-UseSSL</span><span class="sxs-lookup"><span data-stu-id="1c177-549">-UseSSL</span></span>

<span data-ttu-id="1c177-550">指示此 cmdlet 使用安全套接字层 (SSL) 协议来建立与远程计算机的连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-550">Indicates that this cmdlet uses the Secure Sockets Layer (SSL) protocol to establish a connection to the remote computer.</span></span> <span data-ttu-id="1c177-551">默认情况下，不使用 SSL。</span><span class="sxs-lookup"><span data-stu-id="1c177-551">By default, SSL isn't used.</span></span>

<span data-ttu-id="1c177-552">WS-Management 加密通过网络传输的所有 PowerShell 内容。</span><span class="sxs-lookup"><span data-stu-id="1c177-552">WS-Management encrypts all PowerShell content transmitted over the network.</span></span> <span data-ttu-id="1c177-553">**UseSSL** 参数是一种额外的保护措施，它通过 HTTPS 而不是 HTTP 来发送数据。</span><span class="sxs-lookup"><span data-stu-id="1c177-553">The **UseSSL** parameter is an additional protection that sends the data across an HTTPS, instead of HTTP.</span></span>

<span data-ttu-id="1c177-554">如果使用此参数，但 SSL 在用于此命令的端口上不可用，则该命令将失败。</span><span class="sxs-lookup"><span data-stu-id="1c177-554">If you use this parameter, but SSL isn't available on the port that's used for the command, the command fails.</span></span>
...
### <span data-ttu-id="1c177-555">-VMId</span><span class="sxs-lookup"><span data-stu-id="1c177-555">-VMId</span></span>

<span data-ttu-id="1c177-556">指定虚拟机的 Id 的数组。</span><span class="sxs-lookup"><span data-stu-id="1c177-556">Specifies an array of IDs of virtual machines.</span></span>
...
### <span data-ttu-id="1c177-557">-VMName</span><span class="sxs-lookup"><span data-stu-id="1c177-557">-VMName</span></span>

<span data-ttu-id="1c177-558">指定一个虚拟机的名称数组。</span><span class="sxs-lookup"><span data-stu-id="1c177-558">Specifies an array of names of virtual machines.</span></span>
...
### <span data-ttu-id="1c177-559">-子系统</span><span class="sxs-lookup"><span data-stu-id="1c177-559">-Subsystem</span></span>

<span data-ttu-id="1c177-560">指定用于新的 **PSSession** 的 SSH 子系统。</span><span class="sxs-lookup"><span data-stu-id="1c177-560">Specifies the SSH subsystem used for the new **PSSession** .</span></span>

<span data-ttu-id="1c177-561">此项指定要在目标上使用的子系统（如 sshd_config 中所定义）。</span><span class="sxs-lookup"><span data-stu-id="1c177-561">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="1c177-562">子系统使用预定义参数启动特定版本的 PowerShell。</span><span class="sxs-lookup"><span data-stu-id="1c177-562">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="1c177-563">如果远程计算机上不存在指定的子系统，则该命令将失败。</span><span class="sxs-lookup"><span data-stu-id="1c177-563">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="1c177-564">如果未使用此参数，则默认值为 "powershell" 子系统。</span><span class="sxs-lookup"><span data-stu-id="1c177-564">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="1c177-565">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="1c177-565">CommonParameters</span></span>

<span data-ttu-id="1c177-566">此 cmdlet 支持以下常见参数：-Debug、-ErrorAction、-ErrorVariable、-InformationAction、-InformationVariable、-OutVariable、-OutBuffer、-PipelineVariable、-Verbose、-WarningAction 和 -WarningVariable。</span><span class="sxs-lookup"><span data-stu-id="1c177-566">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="1c177-567">有关详细信息，请参阅 [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216)。</span><span class="sxs-lookup"><span data-stu-id="1c177-567">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="1c177-568">输入</span><span class="sxs-lookup"><span data-stu-id="1c177-568">INPUTS</span></span>

### <span data-ttu-id="1c177-569">System.object。</span><span class="sxs-lookup"><span data-stu-id="1c177-569">System.Management.Automation.ScriptBlock</span></span>

<span data-ttu-id="1c177-570">可以通过管道将脚本块中的命令传递给 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-570">You can pipe a command in a script block to `Invoke-Command`.</span></span> <span data-ttu-id="1c177-571">使用 `$Input` 自动变量来表示命令中的输入对象。</span><span class="sxs-lookup"><span data-stu-id="1c177-571">Use the `$Input` automatic variable to represent the input objects in the command.</span></span>

## <span data-ttu-id="1c177-572">输出</span><span class="sxs-lookup"><span data-stu-id="1c177-572">OUTPUTS</span></span>

### <span data-ttu-id="1c177-573">PSRemotingJob、、或调用的命令的输出中的命令的输出。</span><span class="sxs-lookup"><span data-stu-id="1c177-573">System.Management.Automation.PSRemotingJob, System.Management.Automation.Runspaces.PSSession, or the output of the invoked command</span></span>

<span data-ttu-id="1c177-574">如果使用 **AsJob** 参数，则此 cmdlet 将返回一个作业对象。</span><span class="sxs-lookup"><span data-stu-id="1c177-574">This cmdlet returns a job object, if you use the **AsJob** parameter.</span></span> <span data-ttu-id="1c177-575">如果指定 **InDisconnectedSession** 参数，则 `Invoke-Command` 将返回 **PSSession** 对象。</span><span class="sxs-lookup"><span data-stu-id="1c177-575">If you specify the **InDisconnectedSession** parameter, `Invoke-Command` returns a **PSSession** object.</span></span> <span data-ttu-id="1c177-576">否则，它将返回调用的命令的输出，这是 **ScriptBlock** 参数的值。</span><span class="sxs-lookup"><span data-stu-id="1c177-576">Otherwise, it returns the output of the invoked command, which is the value of the **ScriptBlock** parameter.</span></span>

## <span data-ttu-id="1c177-577">注释</span><span class="sxs-lookup"><span data-stu-id="1c177-577">NOTES</span></span>

<span data-ttu-id="1c177-578">在 Windows Vista 和更高版本的 Windows 操作系统上，若要使用的 **ComputerName** 参数 `Invoke-Command` 在本地计算机上运行命令，则必须使用 "以 **管理员身份运行** " 选项运行 PowerShell。</span><span class="sxs-lookup"><span data-stu-id="1c177-578">On Windows Vista, and later versions of the Windows operating system, to use the **ComputerName** parameter of `Invoke-Command` to run a command on the local computer, you must run PowerShell using the **Run as administrator** option.</span></span>

<span data-ttu-id="1c177-579">在多台计算机上运行命令时，PowerShell 会按照它们在列表中的显示顺序连接到这些计算机。</span><span class="sxs-lookup"><span data-stu-id="1c177-579">When you run commands on multiple computers, PowerShell connects to the computers in the order in which they appear in the list.</span></span> <span data-ttu-id="1c177-580">但是，命令输出会按从远程计算机接收的顺序显示，这可能会有所不同。</span><span class="sxs-lookup"><span data-stu-id="1c177-580">However, the command output is displayed in the order that it's received from the remote computers, which might be different.</span></span>

<span data-ttu-id="1c177-581">命令结果中将包含由运行的命令产生的错误 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="1c177-581">Errors that result from the command that `Invoke-Command` runs are included in the command results.</span></span>
<span data-ttu-id="1c177-582">在本地命令中可能是终止错误的错误在远程命令中将视作非终止错误。</span><span class="sxs-lookup"><span data-stu-id="1c177-582">Errors that would be terminating errors in a local command are treated as non-terminating errors in a remote command.</span></span> <span data-ttu-id="1c177-583">此策略可确保一台计算机上的终止错误不会在运行它的所有计算机上关闭该命令。</span><span class="sxs-lookup"><span data-stu-id="1c177-583">This strategy makes sure that terminating errors on one computer don't close the command on all computers on which it's run.</span></span> <span data-ttu-id="1c177-584">即使在一台计算机上运行远程命令，也使用这种做法。</span><span class="sxs-lookup"><span data-stu-id="1c177-584">This practice is used even when a remote command is run on a single computer.</span></span>

<span data-ttu-id="1c177-585">如果远程计算机不在本地计算机信任的域中，则计算机可能无法对用户的凭据进行身份验证。</span><span class="sxs-lookup"><span data-stu-id="1c177-585">If the remote computer isn't in a domain that the local computer trusts, the computer might not be able to authenticate the user's credentials.</span></span> <span data-ttu-id="1c177-586">若要将远程计算机添加到 WS-MANAGEMENT 中的受信任主机列表中，请在提供程序中使用以下命令 `WSMAN` ，其中， `<Remote-Computer-Name>` 是远程计算机的名称：</span><span class="sxs-lookup"><span data-stu-id="1c177-586">To add the remote computer to the list of trusted hosts in WS-Management, use the following command in the `WSMAN` provider, where `<Remote-Computer-Name>` is the name of the remote computer:</span></span>

`Set-Item -Path WSMan:\Localhost\Client\TrustedHosts -Value \<Remote-Computer-Name\>`

<span data-ttu-id="1c177-587">当你使用 **InDisconnectedSession** 参数断开 **PSSession** 的连接时，会话状态将 **断开连接** ，并且可用性为 **None** 。</span><span class="sxs-lookup"><span data-stu-id="1c177-587">When you disconnect a **PSSession** using the **InDisconnectedSession** parameter, the session state is **Disconnected** and the availability is **None** .</span></span> <span data-ttu-id="1c177-588">**State** 属性的值是相对于当前会话的。</span><span class="sxs-lookup"><span data-stu-id="1c177-588">The value of the **State** property is relative to the current session.</span></span> <span data-ttu-id="1c177-589">如果值为 "已 **断开连接** "，则意味着 **PSSession** 未连接到当前会话。</span><span class="sxs-lookup"><span data-stu-id="1c177-589">A value of **Disconnected** means that the **PSSession** isn't connected to the current session.</span></span> <span data-ttu-id="1c177-590">但是，它并不意味着 **PSSession** 会与所有会话断开连接。</span><span class="sxs-lookup"><span data-stu-id="1c177-590">However, it doesn't mean that the **PSSession** is disconnected from all sessions.</span></span> <span data-ttu-id="1c177-591">它可能连接到另一个会话。</span><span class="sxs-lookup"><span data-stu-id="1c177-591">It might be connected to a different session.</span></span> <span data-ttu-id="1c177-592">若要确定是否可以连接或重新连接到该会话，请使用 **Availability** 属性。</span><span class="sxs-lookup"><span data-stu-id="1c177-592">To determine whether you can connect or reconnect to the session, use the **Availability** property.</span></span>

<span data-ttu-id="1c177-593">**Availability** 的 **None** 值指示可连接到 PSSession。</span><span class="sxs-lookup"><span data-stu-id="1c177-593">An **Availability** value of **None** indicates that you can connect to the session.</span></span> <span data-ttu-id="1c177-594">值为 " **忙碌** " 表示无法连接到 **PSSession** ，因为它已连接到另一个会话。</span><span class="sxs-lookup"><span data-stu-id="1c177-594">A value of **Busy** indicates that you can't connect to the **PSSession** because it's connected to another session.</span></span> <span data-ttu-id="1c177-595">有关会话 **状态** 属性的值的详细信息，请参阅 [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate)。</span><span class="sxs-lookup"><span data-stu-id="1c177-595">For more information about the values of the **State** property of sessions, see [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span></span> <span data-ttu-id="1c177-596">有关会话的 **可用性** 属性值的详细信息，请参阅 [runspacestate](/dotnet/api/system.management.automation.runspaces.runspaceavailability)。</span><span class="sxs-lookup"><span data-stu-id="1c177-596">For more information about the values of the **Availability** property of sessions, see [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span></span>

<span data-ttu-id="1c177-597">从 PowerShell 6.0 开始包括 **HostName** 和 **SSHConnection** 参数。</span><span class="sxs-lookup"><span data-stu-id="1c177-597">The **HostName** and **SSHConnection** parameters were included starting with PowerShell 6.0.</span></span> <span data-ttu-id="1c177-598">添加它们是为了提供基于 (SSH) 安全外壳的 PowerShell 远程处理。</span><span class="sxs-lookup"><span data-stu-id="1c177-598">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="1c177-599">在多个平台上支持 PowerShell 和 SSH (Windows、Linux、macOS) 和 PowerShell 远程处理在安装和配置 PowerShell 和 SSH 的这些平台上运行。</span><span class="sxs-lookup"><span data-stu-id="1c177-599">PowerShell and SSH are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting works over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="1c177-600">这不同于以前仅限 Windows 的远程处理，该远程处理基于 WinRM，许多 WinRM 特定功能和限制不适用。</span><span class="sxs-lookup"><span data-stu-id="1c177-600">This is separate from the previous Windows only remoting that is based on WinRM and many of the WinRM specific features and limitations don't apply.</span></span> <span data-ttu-id="1c177-601">例如，目前不支持基于 WinRM 的配额、会话选项、自定义终结点配置和断开/重新连接功能。</span><span class="sxs-lookup"><span data-stu-id="1c177-601">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="1c177-602">有关如何设置 PowerShell SSH 远程处理的详细信息，请参阅 [通过 SSH 进行 Powershell 远程处理](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core)。</span><span class="sxs-lookup"><span data-stu-id="1c177-602">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="1c177-603">相关链接</span><span class="sxs-lookup"><span data-stu-id="1c177-603">RELATED LINKS</span></span>

[<span data-ttu-id="1c177-604">about_PSSessions</span><span class="sxs-lookup"><span data-stu-id="1c177-604">about_PSSessions</span></span>](./About/about_PSSessions.md)

[<span data-ttu-id="1c177-605">about_Remote</span><span class="sxs-lookup"><span data-stu-id="1c177-605">about_Remote</span></span>](./About/about_Remote.md)

[<span data-ttu-id="1c177-606">about_Remote_Disconnected_Sessions</span><span class="sxs-lookup"><span data-stu-id="1c177-606">about_Remote_Disconnected_Sessions</span></span>](./About/about_Remote_Disconnected_Sessions.md)

[<span data-ttu-id="1c177-607">about_Remote_Troubleshooting</span><span class="sxs-lookup"><span data-stu-id="1c177-607">about_Remote_Troubleshooting</span></span>](./About/about_remote_troubleshooting.md)

[<span data-ttu-id="1c177-608">about_Remote_Variables</span><span class="sxs-lookup"><span data-stu-id="1c177-608">about_Remote_Variables</span></span>](./About/about_Remote_Variables.md)

[<span data-ttu-id="1c177-609">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="1c177-609">about_Scopes</span></span>](./About/about_scopes.md)

[<span data-ttu-id="1c177-610">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="1c177-610">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="1c177-611">Exit-PSSession</span><span class="sxs-lookup"><span data-stu-id="1c177-611">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="1c177-612">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="1c177-612">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="1c177-613">Invoke-Item</span><span class="sxs-lookup"><span data-stu-id="1c177-613">Invoke-Item</span></span>](../Microsoft.PowerShell.Management/Invoke-Item.md)

[<span data-ttu-id="1c177-614">New-PSSession</span><span class="sxs-lookup"><span data-stu-id="1c177-614">New-PSSession</span></span>](New-PSSession.md)

[<span data-ttu-id="1c177-615">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="1c177-615">Remove-PSSession</span></span>](Remove-PSSession.md)

[<span data-ttu-id="1c177-616">WSMan 提供程序</span><span class="sxs-lookup"><span data-stu-id="1c177-616">WSMan Provider</span></span>](../Microsoft.WsMan.Management/About/about_WSMan_Provider.md)
