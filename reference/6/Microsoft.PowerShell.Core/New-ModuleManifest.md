...
`New-ModuleManifest``.psd1`按如下所示创建模块清单 () 文件编码：

- 在 Windows 上运行的 PowerShell 6.0 使用 **UTF16** 。
- 在非 Windows 平台上运行的 PowerShell 6.0 使用 **UTF8NoBOM** 。
- 在 Windows 和非 Windows 平台上运行的 PowerShell 6.1 和更高版本使用 **UTF8NoBOM** 。
