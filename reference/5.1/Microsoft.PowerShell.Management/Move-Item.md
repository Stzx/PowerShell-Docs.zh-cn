...
`Move-Item`Cmdlet 将项（包括其属性、内容和子项）从一个位置移动到另一个位置。
但这些位置必须由同一提供程序支持。
...
此命令将驱动器中的 "Test.txt" 文件移 `C:` 到 "E:\Temp" 目录，并将其从 "test.txt" 重命名为 "tst.txt"。
...
此命令将 "C：\Temp" 目录及其内容移动到 "C:\Logs" 目录。
...
此命令将当前目录中的所有文本文件 ( "* .txt" ) ， (用点 ( "." 表示。将 # A5 ) 到 "C:\Logs" 目录。
...
该命令使用 `Get-ChildItem` cmdlet 来获取当前目录中的所有子项目， (由 \[ \]) 及其子目录（其 *文件扩展名为 ".txt"）表示。它使用 recursive **Recurse** 参数将检索设置为 recursive，并使用 Include 参数将检索限制为 "* .txt" 文件。
...
注意：在 Windows PowerShell 2.0 中，使用 cmdlet 的 **递归** 参数时 `Get-ChildItem` ， **Path** 参数的值必须是一个容器。
...
此命令将 "HKLM\Software" 中 "MyCompany" 注册表项中的注册表项和值移动到 "MyNewCompany" 键。
...
指定为字符串数组，此 cmdlet 将在操作中移动的一个或一些项。
此参数值使 **Path** 参数有效。
请输入路径元素或模式，例如“*.txt”。
允许使用通配符。
