...
`Format-Custom` 旨在显示不只是表格或只是列表的视图。 你可以使用 PowerShell 目录的文件中定义的视图 `*format.PS1XML` ，也可以在新的 types.ps1xml 文件中创建自己的视图，并使用 `Update-FormatData` cmdlet 将其添加到 powershell。
