...
此示例使用 **IncludeEqual** 和 **ExcludeDifferent** 参数来比较两个文本文件中的每行内容。

由于该命令使用 **ExcludeDifferent** 参数，因此输出只包含两个文件中包含的行，如 **SideIndicator** () 所示 `==` 。
...
如果对象不同，则 `Compare-Object` 使用 SideIndicator 属性包装包装中的不同对象， `PSCustomObject` 以引用差异。 **SideIndicator**
