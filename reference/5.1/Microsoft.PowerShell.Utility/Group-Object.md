...
`Group-Object` 不要求分组的对象具有相同的 Microsoft .NET Framework 类型。 分组不同 .NET Framework 类型的对象时， `Group-Object` 将使用以下规则：

- 相同的属性名和类型。

  如果对象具有带指定名称的属性，并且属性值具有相同的 .NET Framework 类型，则通过使用相同类型的对象适用的规则对属性值进行分组。

- 相同的属性名称和不同类型。

  如果对象具有具有指定名称的属性，但属性值在不同的对象中具有不同的 .NET Framework 类型，则 `Group-Object` 使用属性的第一个匹配项的 .NET Framework 类型作为该属性组的 .NET Framework 类型。 当对象具有不同类型的属性时，会将属性值转换为该组的类型。 如果类型转换失败，则不会将该对象包含在组中。
