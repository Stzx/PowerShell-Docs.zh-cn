...
## DESCRIPTION
**New-Alias** cmdlet 在当前的 Windows PowerShell 会话中创建新别名。
在退出会话或关闭 Windows PowerShell 后，不会保存通过使用 **New-Alias** 创建的别名。
...
* 若要创建新的别名，请使用 Set-Alias 或 New-Alias。 若要更改别名，请使用 **Set-Alias**。 若要删除别名，请使用 Remove-Item。
