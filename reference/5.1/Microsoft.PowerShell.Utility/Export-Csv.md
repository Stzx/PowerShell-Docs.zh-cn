...
`Get-Process`Cmdlet 将获取 **进程** 对象。 进程对象将通过管道向下发送到 `Export-Csv` cmdlet。 `Export-Csv` 将进程对象转换为一系列 CSV 字符串。
**Path** 参数指定将 Processes.csv 文件保存到当前目录中。 `Get-Content`Cmdlet 使用 **Path** 参数显示位于当前目录中的文件。
...
`Export-CSV`Cmdlet 将提交的对象转换为一系列 CSV 字符串，并将它们保存在指定的文本文件中。 你可以使用将 `Export-CSV` 对象保存在 csv 文件中，然后使用 `Import-Csv` CMDLET 从 csv 文件创建对象。

在 CSV 文件中，通过以逗号分隔的对象属性值列表来表示每个对象。 使用 **ToString ( # B1** 方法将属性值转换为字符串。 字符串由属性值名称表示。 "Export-CSV 不导出对象的方法。
