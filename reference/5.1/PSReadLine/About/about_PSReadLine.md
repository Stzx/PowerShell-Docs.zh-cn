...
PSReadLine 2.0 为 PowerShell 控制台提供了强大的命令行编辑体验。 提供以下功能：
...
此函数由 Get-PSReadLineKeyHandler 使用，并可能在自定义键绑定中不起作用。
...
PSReadLine 维护一个历史记录文件，其中包含在命令行中输入的所有命令和数据。 这可能包含敏感数据（包括密码）。 例如，如果使用 cmdlet，则 `ConvertTo-SecureString` 密码将作为纯文本记录到历史记录文件中。 历史记录文件是一个名为的文件 `$($host.Name)_history.txt` 。 在 Windows 系统上，历史记录文件存储在中 `$env:APPDATA\Microsoft\Windows\PowerShell\PSReadLine` 。
