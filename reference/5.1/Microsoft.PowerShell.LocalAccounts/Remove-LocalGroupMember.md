...
## DESCRIPTION
**LocalGroupMember** cmdlet 将从本地组中删除用户或组。
...
此命令从本地管理员组中删除多个成员。
此 cmdlet 删除的成员包括本地用户帐户、Microsoft 帐户、Azure Active Directory 帐户和域组。
此示例在 Outlook.com 处为帐户的用户名使用占位符值。

## PARAMETERS

### -Group
指定此 cmdlet 从中删除成员的安全组。
...
### -成员
指定此 cmdlet 从安全组中删除的用户或组的数组。
可以按名称、安全 ID (SID) 或 **LocalPrincipal** 对象指定用户或组。
...
### SecurityAccountsManager. LocalPrincipal、SecurityIdentifier 和的数据库的安全层。
