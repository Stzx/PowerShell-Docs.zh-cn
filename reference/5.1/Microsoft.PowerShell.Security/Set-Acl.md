...
建立或更改项的中心访问策略。
在计算机上输入中心访问策略的 CAP ID 或友好名称。

从 Windows Server 2012 开始，管理员可以使用 Active Directory 和组策略为用户和组设置中心访问策略。
有关详细信息，请参阅 [动态访问控制：方案概述](/windows-server/identity/solution-guides/dynamic-access-control--scenario-overview)。
