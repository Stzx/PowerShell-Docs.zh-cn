...
Invoke-WSManAction 对由 RESOURCE_URI 指定的对象运行一项操作，其中参数由键值对指定。
...
因此，将创建一个新进程来运行记事本，新进程的当前目录设置为 "C：" \" 。
...
此命令调用对应于后台处理程序服务的 Win32_Service WMI 类实例的 StartService 方法。
由于指定了 ComputerName 参数，因此该命令针对 server01 远程计算机运行。
...
指定可帮助修改管理资源的哈希表。
使用 ResourceURI 和 SelectorSet 参数指定管理资源。
ValueSet 参数的值必须为哈希表。
