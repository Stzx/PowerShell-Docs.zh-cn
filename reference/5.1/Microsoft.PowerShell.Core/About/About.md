...
描述语句如何将 `Continue` 程序流立即返回到程序循环的顶部。
...
简要介绍了 PowerShell 所需状态配置 (DSC) 日志资源。
...
Windows PowerShell 会创建一个名为 "Windows PowerShell" 的 Windows 事件日志来记录 Windows PowerShell 事件。
...
`Format.ps1xml`Powershell 中的文件在 powershell 控制台中定义对象的默认显示。
...
介绍如何使用 PowerShell.exe 命令行工具。 显示语法并描述命令行开关。
...
介绍 PowerShell 管理单元，并演示如何使用和管理这些管理单元。
...
提供有关 WMI) 和 PowerShell (Windows Management Instrumentation 的背景信息。

### [about_WQL](about_WQL.md)
介绍 WMI 查询语言 (WQL) ，可用于获取 PowerShell 中的 WMI 对象。
