...
- **AllUsers** 位置 `$env:PROGRAMFILES\WindowsPowerShell\Modules` 在 Windows 上。
...
有关 PowerShell 管理单元的详细信息，请参阅 [about_PSSnapins](about_PSSnapins.md)。
...
你还可以使用组策略设置，启用模块日志记录，以便在所有 PowerShell 会话中启用模块日志记录。 有关详细信息，请参阅 [about_EventLogs](about_EventLogs.md) 和 [about_Group_Policy_Settings](about_Group_Policy_Settings.md)。
