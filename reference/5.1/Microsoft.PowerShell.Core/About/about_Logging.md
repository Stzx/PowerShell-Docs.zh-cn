...
可以使用 Windows 事件查看器查看 PowerShell 日志。 事件日志位于 "应用程序" 和 "服务日志" 组中，并命名为 `Microsoft-Windows-PowerShell` 。 关联的 ETW 提供程序 `GUID` 为 `{A0C1853B-5C40-4B15-8766-3CF1C58F985A}` 。

启用脚本块日志记录时，PowerShell 会将以下事件记录到 `Microsoft-Windows-PowerShell/Operational` 日志中：
