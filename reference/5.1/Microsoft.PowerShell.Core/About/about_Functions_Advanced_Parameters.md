...
下面的示例在 **ComputerName** `Computer` 参数集、参数集中的 **用户名** 参数以及 `User` 这两个参数集中的 **Summary** 参数中声明了 ComputerName 参数。
...
使用带有 **ValueFromRemainingArguments** 的集合有一个已知问题，其中传入的集合被视为单个元素。

下面的示例演示了此已知问题。 **剩余** 的参数应在索引 **0** 处包含 **一个** 参数，在 **索引 1** 处包含 **2 个** 参数。
相反，这两个元素合并到一个实体中。
...
**ValidateRange** 属性指定每个参数或变量值的数值范围。 如果任何值超出此范围，则 PowerShell 会生成错误。 在下面的示例中，" **尝试** " 参数的值必须介于0和10之间。
...
例如，多个提供程序 cmdlet 的参数仅在提供程序驱动器中使用 cmdlet 时，或在提供程序驱动器的特定路径中可用。 例如， **Encoding** `Add-Content` `Get-Content` `Set-Content` 仅当在文件系统驱动器中使用编码参数时，才在、和 cmdlet 上使用该参数。
