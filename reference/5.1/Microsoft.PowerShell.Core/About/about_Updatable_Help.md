...
## <a name="update-help-in-the-powershell-ise"></a>在 PowerShell ISE 中更新帮助

你还可以使用 PowerShell 集成脚本环境 (ISE) 中的 "帮助" 菜单中的 " **更新 PowerShell 帮助** " 项来更新帮助。

**Update PowerShell 帮助** 项运行 `Update-Help` 没有参数的命令。
...
`Update-Help`和 `Save-Help` cmdlet 还具有一个 **Credential** 参数，该参数使你可以在远程计算机上运行该命令并访问第三台计算机上的文件共享。 仅当使用的 SourcePath 或 **LiteralPath** 参数和的 **Credential** `Update-Help` **DestinationPath** 或 **LiteralPath** 参数时，Credential 参数才有效 `Save-Help` 。
