...
使用 Enter-PSSession cmdlet 来启动与远程计算机的交互式会话。 可以使用 Enter-PSSession 的 ComputerName 参数为交互式会话建立临时连接。 或者，你可以使用 Session 参数 (PSSession) 在 Windows PowerShell 会话中运行交互式会话。
...
你可以使用包含 job 名词的 cmdlet (作业 cmdlet) 管理任何 cmdlet 创建的任何作业。 许多具有 AsJob 参数的 cmdlet 不使用 Windows PowerShell 远程处理，因此您甚至可以在未配置为进行远程处理并且不满足远程处理要求的计算机上使用它们。
