...
由于大多数 cmdlet 和函数使用 **默认的风险** 值 **ConfirmImpact** ，而默认值为 "高"，因此 `$ConfirmPreference` 很 **High** 少发生自动确认。 但是，可以通过将的值更改为 "中" `$ConfirmPreference` 或 **Medium** " **低** " 来激活自动确认。
...
此示例显示 `$DebugPreference` 变量的默认值 **SilentlyContinue** 的效果。 默认情况下， `Write-Debug` cmdlet 的调试消息不会显示并且处理将继续进行。 使用 **Debug** 参数时，它将覆盖单个命令的首选项。 系统将提示用户进行确认。
...
- **NormalView** ： (默认) 为大多数用户设计的详细视图。 包含错误说明和错误中涉及的对象的名称。
...
此示例演示当的值 `$ErrorView` 为默认值 **NormalView** 时，如何显示错误。 `Get-ChildItem` 用于查找不存在的文件。
...
- **继续** ：显示信息性消息，并继续运行。
- **挂起** ：执行命令后，自动挂起工作流作业 `Write-Information` ，以允许用户在继续操作之前查看消息。 可以按用户的意愿恢复工作流。
...
确定 PowerShell 会话中允许的别名数。 默认值为 **4096** ，应足以满足大多数使用的需要。 你可以调整 `$MaximumAliasCount` 以满足你的需求。
...
确定给定会话中允许的 PowerShell 驱动器数。 例如，文件系统驱动器和 PowerShell 提供程序公开的数据存储区，它们显示为驱动器，如 `Alias:` 和 `HKLM:` 驱动器。
...
表示每个保留错误的对象存储在 `$Error` 自动变量中。 `$Error` 包含错误记录对象的数组。 最近的错误是数组中的第一个对象 `$Error[0]` 。

若要对系统中的错误进行计数，请使用 `$Error` 数组的 **count** 属性。

```powershell
$Error.count
```

若要显示特定错误，请使用 `[0]` 数组表示法来查看最近的错误。
...
**Force** 参数重写 **ErrorRecord** 对象的特殊格式，并恢复为传统格式。 若要显示 **ErrorRecord** 对象的属性，请键入以下命令：

```powershell
$Error[0] | Format-List -Property * -Force
```

在此示例中， `$Error.Count` 显示错误数。 若要从错误历史记录中删除所有错误，请使用 `Clear` 错误数组的方法。
...
若要查找错误数组的所有属性和方法，请使用 `Get-Member` 带有 **InputObject** 参数的 cmdlet。 当使用 **InputObject** 参数时，将 `Get-Member` 显示集合的属性和方法。

```powershell
Get-Member -InputObject $Error
```

将对象的集合传递给时 `Get-Member` ，会 `Get-Member` 显示集合中对象的属性和方法。
...
若要查看会话中的函数，请使用 `Function:` powershell 提供程序公开的 powershell 驱动器 `Function` 。 有关提供程序的详细信息 `Function` ，请 [about_Function_Provider](about_Function_Provider.md)。
...
确定给定会话中允许的变量数，包括自动变量、首选项变量以及您在命令和脚本中创建的变量。

**有效值** ： 1024-32768 (`Int32`) 

**默认值** ：4096

若要查看会话中的变量，请使用 `Get-Variable` cmdlet 以及 powershell `Variable:` 驱动器和 powershell `Variable` 提供程序的功能。 有关信息，请参阅 [about_Variable_Provider](about_Variable_Provider.md)。
...
**默认值** ： ASCIIEncoding 对象 (ASCIIEncoding) 
...
你可以使用 **ConfigurationName** `New-PSSession` 、 `Enter-PSSession` 或 cmdlet 的 ConfigurationName 参数覆盖默认值，并为特定会话选择不同的会话配置 `Invoke-Command` 。
...
此示例使用带有 **Verbose** `$false` 替代 **Inquire** 值的值的详细参数。 系统不会提示用户并且不显示消息。
...
此示例演示在的值为 False 时使用 **WhatIf** 参数的效果 `$WhatIfPreference` 。 **False**
...
此示例演示如何在的值为 True 时删除文件 `$WhatIfPreference` 。 **True** 它使用值为的 **WhatIf** 参数 `$false` 。 使用 `Get-ChildItem` 验证是否已删除该文件。
