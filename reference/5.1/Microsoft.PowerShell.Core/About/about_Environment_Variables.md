...
_进程_ 范围包含当前进程或 PowerShell 会话中可用的环境变量。 此变量列表继承自父进程，并由 _计算机_ 和 _用户_ 作用域中的变量构造。
...
- PSModuleAnalysisCachePath
  ...
  缓存的默认文件名为 `ModuleAnalysisCache` 。 若要更改缓存的默认位置，请在启动 PowerShell 之前设置环境变量。 对此环境变量所做的更改只会影响子进程。
