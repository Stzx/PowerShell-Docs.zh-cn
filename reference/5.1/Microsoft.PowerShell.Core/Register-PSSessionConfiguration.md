...
- 空。 默认情况下，不向会话添加任何模块。 使用此 cmdlet 的参数将模块、函数、脚本和其他功能添加到会话。
- 默认。 向会话添加 ""。 此模块包括 `Import-Module` cmdlet，用户可以使用该 cmdlet 导入其他模块，除非显式禁止该 cmdlet。
- RestrictedRemoteServer. 仅包含以下 cmdlet： `Exit-PSSession` 、 `Get-Command` 、 `Get-FormatData` 、、、 `Get-Help` `Measure-Object` `Out-Default` 和 `Select-Object` 。 使用脚本或程序集，或者会话配置文件中的键，来将模块、函数、脚本和其他功能添加到会话。
