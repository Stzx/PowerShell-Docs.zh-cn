...
>
> 若要下载或更新 PowerShell 安装目录中的模块的帮助文件 (`$PSHOME\Modules`) （包括 PowerShell Core 模块），请使用 "以管理员身份运行" 选项启动 powershell。
> 例如：`Start-Process powershell.exe -Verb RunAs`。
>
> 你还可以使用 Windows PowerShell 集成脚本环境 (ISE) 中的 "更新 Windows PowerShell 帮助" 菜单项来更新帮助文件。
>
> 更新 Windows PowerShell 帮助项运行 `Update-Help` 没有参数的 cmdlet。
> 若要为目录中的模块更新帮助 `$PSHOME` ，请使用 "以管理员身份运行" 选项启动 Windows PowerShell ISE。
...
此示例将创建一个计划作业，该作业将每天凌晨3:00 为所有模块更新帮助。
...
`Register-ScheduledJob`Cmdlet 可创建运行命令的计划作业 `Update-Help` 。 该命令使用 **Credential** 参数，以便 `Update-Help` 通过使用计算机上 Administrators 组的成员的凭据来运行。 **Trigger** 参数的值是一个 `New-JobTrigger` 命令，该命令创建一个作业触发器，该触发器每天凌晨3:00 启动作业。

若要运行此 `Register-ScheduledJob` 命令，请使用 "以 **管理员身份运行** " 选项启动 PowerShell。 PowerShell 会提示你输入 **Credential** 参数中指定的用户的密码。 凭据随计划作业一起存储。 作业运行时，不会出现提示。

你可以使用 `Get-ScheduledJob` cmdlet 来查看计划作业，使用 `Set-ScheduledJob` cmdlet 对其进行更改，并使用 `Unregister-ScheduledJob` cmdlet 将其删除。 你还可以在以下路径中查看和管理任务计划程序内的计划作业：

`Task Scheduler Library\Microsoft\Windows\PowerShell\ScheduledJobs`.

### 示例5：从文件共享更新多台计算机上的帮助文件
...
指定更新的帮助文件的文件夹，而不是从 internet 下载。 如果已使用 **SourcePath** `Save-Help` cmdlet 将帮助文件下载到目录，请使用此参数或 SourcePath。
...
默认情况下，会 `Update-Help` 从 internet 下载更新的帮助文件。 使用 **SourcePath** 该 `Save-Help` cmdlet 将已更新的帮助文件下载到目录时，请使用 SourcePath。
