...
## <a name="where-can-i-install-powershell"></a><span data-ttu-id="d36bb-116">可将 PowerShell 安装在何处？</span><span class="sxs-lookup"><span data-stu-id="d36bb-116">Where can I install PowerShell?</span></span>
...
## <a name="running-powershell-7"></a><span data-ttu-id="d36bb-132">运行 PowerShell 7</span><span class="sxs-lookup"><span data-stu-id="d36bb-132">Running PowerShell 7</span></span>
...
<span data-ttu-id="d36bb-157">Parallel 参数指定为每个输入日志名称并行运行的脚本块。</span><span class="sxs-lookup"><span data-stu-id="d36bb-157">The **Parallel** parameter specifies the script block that is run in parallel for each input log name.</span></span>
...
<span data-ttu-id="d36bb-172">有关[关于假设情况](/powershell/module/microsoft.powershell.core/about/about_if)的详细信息。</span><span class="sxs-lookup"><span data-stu-id="d36bb-172">For more information [About If](/powershell/module/microsoft.powershell.core/about/about_if).</span></span>
...
<span data-ttu-id="d36bb-211">使用 ConciseView，如果错误不是源自脚本或分析器错误，则它是单行错误消息：</span><span class="sxs-lookup"><span data-stu-id="d36bb-211">With **ConciseView** , if an error is not from a script or parser error, then it's a single line error message:</span></span>
...
<span data-ttu-id="d36bb-223">`Get-Error` cmdlet 支持最新参数，允许你指定当前会话中要显示的错误数。</span><span class="sxs-lookup"><span data-stu-id="d36bb-223">The `Get-Error` cmdlet supports the **Newest** parameter, allowing you to specify how many errors from the current session you wish displayed.</span></span>
...
- <span data-ttu-id="d36bb-270">修复非 Windows 平台上的单元线程状态检查 (#11301)</span><span class="sxs-lookup"><span data-stu-id="d36bb-270">Fix apartment thread state check on non-Windows platforms (#11301)</span></span>
...
- <span data-ttu-id="d36bb-293">将 OutputType 添加到 Get-Error cmdlet 并保留原始类型名称 (#10856)</span><span class="sxs-lookup"><span data-stu-id="d36bb-293">Add OutputType to Get-Error cmdlet and preserve original typenames (#10856)</span></span>
...
- <span data-ttu-id="d36bb-314">在 Windows 上重新添加 Out-Printer 命令 (#10906)</span><span class="sxs-lookup"><span data-stu-id="d36bb-314">Add back Out-Printer command on Windows (#10906)</span></span>
...
- <span data-ttu-id="d36bb-337">修复在设置 -TotalCount 时的 Get-Content -ReadCount 0 行为 (#10749)（感谢 @eugenesmlv！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-337">Fix Get-Content -ReadCount 0 behavior when -TotalCount is set (#10749) (Thanks @eugenesmlv!)</span></span>
...
- <span data-ttu-id="d36bb-358">添加 PowerShell Core 组策略定义 (#10468)</span><span class="sxs-lookup"><span data-stu-id="d36bb-358">Add PowerShell Core group policy definitions (#10468)</span></span>
...
### <a name="code-cleanup"></a><span data-ttu-id="d36bb-377">代码清理</span><span class="sxs-lookup"><span data-stu-id="d36bb-377">Code Cleanup</span></span>
...
- <span data-ttu-id="d36bb-413">使不可靠的 DSC 测试挂起 (#11131)</span><span class="sxs-lookup"><span data-stu-id="d36bb-413">Make unreliable DSC test pending (#11131)</span></span>
...
- <span data-ttu-id="d36bb-417">更新测试以准确描述测试操作。</span><span class="sxs-lookup"><span data-stu-id="d36bb-417">Update tests to accurately describe test actions.</span></span> <span data-ttu-id="d36bb-418">(#10928)（感谢 @romero126！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-418">(#10928) (Thanks @romero126!)</span></span>
- <span data-ttu-id="d36bb-419">暂时跳过运行不正常的测试 TestAppDomainProcessExitEvenHandlerNotLeaking (#10827)</span><span class="sxs-lookup"><span data-stu-id="d36bb-419">Temporary skip the flaky test TestAppDomainProcessExitEvenHandlerNotLeaking (#10827)</span></span>
- <span data-ttu-id="d36bb-420">使事件处理程序泄漏测试稳定 (#10790)</span><span class="sxs-lookup"><span data-stu-id="d36bb-420">Make the event handler leaking test stable (#10790)</span></span>
- <span data-ttu-id="d36bb-421">同步 CI YAML 中的大写 (#10767)（感谢 @RDIL！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-421">Sync capitalization in CI YAML (#10767) (Thanks @RDIL!)</span></span>
- <span data-ttu-id="d36bb-422">添加针对事件处理程序泄漏修复的测试 (#10768)</span><span class="sxs-lookup"><span data-stu-id="d36bb-422">Add test for the event handler leaking fix (#10768)</span></span>
- <span data-ttu-id="d36bb-423">添加 Get-ChildItem 测试 (#10507)（感谢 @iSazonov！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-423">Add Get-ChildItem test (#10507) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="d36bb-424">将测试的不明确语言从开关切换到参数以确保准确性 (#10666)（感谢 @romero126！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-424">Replace ambiguous language for tests from switch to parameter for accuracy (#10666) (Thanks @romero126!)</span></span>
- <span data-ttu-id="d36bb-425">向 ForEach-Object -Parallel 测试添加实验性检查 (#10354)（感谢 @KirkMunro！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-425">Add experimental check to ForEach-Object -Parallel tests (#10354) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="d36bb-426">更新 Alpine 验证的测试 (#10428)</span><span class="sxs-lookup"><span data-stu-id="d36bb-426">Update tests for Alpine validation (#10428)</span></span>

### <a name="build-and-package-improvements"></a><span data-ttu-id="d36bb-427">生成和包改进</span><span class="sxs-lookup"><span data-stu-id="d36bb-427">Build and Package Improvements</span></span>

- <span data-ttu-id="d36bb-428">修复协调包生成的 Nuget 包签名 (#11316)</span><span class="sxs-lookup"><span data-stu-id="d36bb-428">Fix Nuget package signing for Coordinated Package build (#11316)</span></span>
- <span data-ttu-id="d36bb-429">更新 PowerShell 库和 NuGet 中的依赖项 (#11323)</span><span class="sxs-lookup"><span data-stu-id="d36bb-429">Update dependencies from PowerShell Gallery and NuGet (#11323)</span></span>
- <span data-ttu-id="d36bb-430">将 Microsoft.ApplicationInsights 从 2.11.0 升级到 2.12.0 (#11305)</span><span class="sxs-lookup"><span data-stu-id="d36bb-430">Bump Microsoft.ApplicationInsights from 2.11.0 to 2.12.0 (#11305)</span></span>
- <span data-ttu-id="d36bb-431">将 Microsoft.CodeAnalysis.CSharp 从 3.3.1 升级到 3.4.0 (#11265)</span><span class="sxs-lookup"><span data-stu-id="d36bb-431">Bump Microsoft.CodeAnalysis.CSharp from 3.3.1 to 3.4.0 (#11265)</span></span>
- <span data-ttu-id="d36bb-432">更新 Debian 10 和 11 的包 (#11236)</span><span class="sxs-lookup"><span data-stu-id="d36bb-432">Updates packages for Debian 10 and 11 (#11236)</span></span>
- <span data-ttu-id="d36bb-433">仅启用 RC 之前的实验性功能 (#11162)</span><span class="sxs-lookup"><span data-stu-id="d36bb-433">Only enable experimental features prior to RC (#11162)</span></span>
- <span data-ttu-id="d36bb-434">更新 macOS 最低版本 (#11163)</span><span class="sxs-lookup"><span data-stu-id="d36bb-434">Update macOS minimum version (#11163)</span></span>
- <span data-ttu-id="d36bb-435">将 NJsonSchema 从 10.0.27 升级到 10.0.28 (#11170)</span><span class="sxs-lookup"><span data-stu-id="d36bb-435">Bump NJsonSchema from 10.0.27 to 10.0.28 (#11170)</span></span>
- <span data-ttu-id="d36bb-436">更新 Preview.5 的 README.md 和 metadata.json 中的链接 (#10854)</span><span class="sxs-lookup"><span data-stu-id="d36bb-436">Updating links in README.md and metadata.json for Preview.5 (#10854)</span></span>
- <span data-ttu-id="d36bb-437">选择 PowerShell 拥有的符合性测试的文件 (#10837)</span><span class="sxs-lookup"><span data-stu-id="d36bb-437">Select the files for compliance tests which are owned by PowerShell (#10837)</span></span>
- <span data-ttu-id="d36bb-438">允许生成 win7x86 .msix 包。</span><span class="sxs-lookup"><span data-stu-id="d36bb-438">Allow win7x86 msix package to build.</span></span> <span data-ttu-id="d36bb-439">（内部 10515）</span><span class="sxs-lookup"><span data-stu-id="d36bb-439">(Internal 10515)</span></span>
- <span data-ttu-id="d36bb-440">允许将语义版本传递给 NormalizeVersion 函数 (#11087)</span><span class="sxs-lookup"><span data-stu-id="d36bb-440">Allow semantic versions to be passed to NormalizeVersion function (#11087)</span></span>
- <span data-ttu-id="d36bb-441">将 .NET Core 框架升级到 3.1-preview.3 (#11079)</span><span class="sxs-lookup"><span data-stu-id="d36bb-441">Bump .NET core framework to 3.1-preview.3 (#11079)</span></span>
- <span data-ttu-id="d36bb-442">在 /src/Modules 中将 PSReadLine 从 2.0.0-beta5 升级到 2.0.0-beta6 (#11078)</span><span class="sxs-lookup"><span data-stu-id="d36bb-442">Bump PSReadLine from 2.0.0-beta5 to 2.0.0-beta6 in /src/Modules (#11078)</span></span>
- <span data-ttu-id="d36bb-443">将 Newtonsoft.Json 从 12.0.2 升级到 12.0.3 (#11037) (#11038)</span><span class="sxs-lookup"><span data-stu-id="d36bb-443">Bump Newtonsoft.Json from 12.0.2 to 12.0.3 (#11037) (#11038)</span></span>
- <span data-ttu-id="d36bb-444">添加 Debian 10、11 和 CentOS 8 包 (#11028)</span><span class="sxs-lookup"><span data-stu-id="d36bb-444">Add Debian 10, 11 and CentOS 8 packages (#11028)</span></span>
- <span data-ttu-id="d36bb-445">使用 ReleaseDate 字段上传 Build-Info Json 文件 (#10986)</span><span class="sxs-lookup"><span data-stu-id="d36bb-445">Upload Build-Info Json file with the ReleaseDate field (#10986)</span></span>
- <span data-ttu-id="d36bb-446">将 .NET Core 框架升级到 3.1-preview.2 (#10993)</span><span class="sxs-lookup"><span data-stu-id="d36bb-446">Bump .NET core framework to 3.1-preview.2 (#10993)</span></span>
- <span data-ttu-id="d36bb-447">启用 x86 MSIX 包的生成 (#10934)</span><span class="sxs-lookup"><span data-stu-id="d36bb-447">Enable build of x86 MSIX package (#10934)</span></span>
- <span data-ttu-id="d36bb-448">更新 build.psm1 中的 dotnet SDK 安装脚本 URL (#10927)</span><span class="sxs-lookup"><span data-stu-id="d36bb-448">Update the dotnet SDK install script URL in build.psm1 (#10927)</span></span>
- <span data-ttu-id="d36bb-449">将 Markdig.Signed 从 0.17.1 升级到 0.18.0 (#10887)</span><span class="sxs-lookup"><span data-stu-id="d36bb-449">Bump Markdig.Signed from 0.17.1 to 0.18.0 (#10887)</span></span>
- <span data-ttu-id="d36bb-450">将 ThreadJob 从 2.0.1 升级到 2.0.2 (#10886)</span><span class="sxs-lookup"><span data-stu-id="d36bb-450">Bump ThreadJob from 2.0.1 to 2.0.2 (#10886)</span></span>
- <span data-ttu-id="d36bb-451">更新 AppX 清单和打包模块以符合 MS Store 要求 (#10878)</span><span class="sxs-lookup"><span data-stu-id="d36bb-451">Update AppX Manifest and Packaging module to conform to MS Store requirements (#10878)</span></span>
- <span data-ttu-id="d36bb-452">将 PowerShell SDK 的包引用更新为 preview.5（内部 10295）</span><span class="sxs-lookup"><span data-stu-id="d36bb-452">Update package reference for PowerShell SDK to preview.5 (Internal 10295)</span></span>
- <span data-ttu-id="d36bb-453">更新 ThirdPartyNotices.txt (#10834)</span><span class="sxs-lookup"><span data-stu-id="d36bb-453">Update ThirdPartyNotices.txt (#10834)</span></span>
- <span data-ttu-id="d36bb-454">将 Microsoft.PowerShell.Native 升级到 7.0.0-preview.3 (#10826)</span><span class="sxs-lookup"><span data-stu-id="d36bb-454">Bump Microsoft.PowerShell.Native to 7.0.0-preview.3 (#10826)</span></span>
- <span data-ttu-id="d36bb-455">将 Microsoft.ApplicationInsights 从 2.10.0 升级到 2.11.0 (#10608)</span><span class="sxs-lookup"><span data-stu-id="d36bb-455">Bump Microsoft.ApplicationInsights from 2.10.0 to 2.11.0 (#10608)</span></span>
- <span data-ttu-id="d36bb-456">将 NJsonSchema 从 10.0.24 升级到 10.0.27 (#10756)</span><span class="sxs-lookup"><span data-stu-id="d36bb-456">Bump NJsonSchema from 10.0.24 to 10.0.27 (#10756)</span></span>
- <span data-ttu-id="d36bb-457">向生成系统添加 MacPorts 支持 (#10736)（感谢 @Lucius-Q-User！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-457">Add MacPorts support to the build system (#10736) (Thanks @Lucius-Q-User!)</span></span>
- <span data-ttu-id="d36bb-458">将 PackageManagement 从 1.4.4 升级到 1.4.5 (#10728)</span><span class="sxs-lookup"><span data-stu-id="d36bb-458">Bump PackageManagement from 1.4.4 to 1.4.5 (#10728)</span></span>
- <span data-ttu-id="d36bb-459">将 NJsonSchema 从 10.0.23 升级到 10.0.24 (#10635)</span><span class="sxs-lookup"><span data-stu-id="d36bb-459">Bump NJsonSchema from 10.0.23 to 10.0.24 (#10635)</span></span>
- <span data-ttu-id="d36bb-460">添加环境变量以区分 MSI 中的客户端/服务器遥测 (#10612)</span><span class="sxs-lookup"><span data-stu-id="d36bb-460">Add environment variable to differentiate client/server telemetry in MSI (#10612)</span></span>
- <span data-ttu-id="d36bb-461">将 PSDesiredStateConfiguration 从 2.0.3 升级到 2.0.4 (#10603)</span><span class="sxs-lookup"><span data-stu-id="d36bb-461">Bump PSDesiredStateConfiguration from 2.0.3 to 2.0.4 (#10603)</span></span>
- <span data-ttu-id="d36bb-462">将 Microsoft.CodeAnalysis.CSharp 从 3.2.1 升级到 3.3.1 (#10607)</span><span class="sxs-lookup"><span data-stu-id="d36bb-462">Bump Microsoft.CodeAnalysis.CSharp from 3.2.1 to 3.3.1 (#10607)</span></span>
- <span data-ttu-id="d36bb-463">更新为 Net Core 3.0 RTM (#10604)（感谢 @bergmeister！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-463">Update to .Net Core 3.0 RTM (#10604) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="d36bb-464">更新 .MSIX 打包，使版本符合 Windows 应用商店要求 (#10588)</span><span class="sxs-lookup"><span data-stu-id="d36bb-464">Update MSIX packaging so the version to Windows Store requirements (#10588)</span></span>
- <span data-ttu-id="d36bb-465">将 PowerShellGet 版本从 2.2 升级到 2.2.1 (#10382)</span><span class="sxs-lookup"><span data-stu-id="d36bb-465">Bump PowerShellGet version from 2.2 to 2.2.1 (#10382)</span></span>
- <span data-ttu-id="d36bb-466">将 PackageManagement 版本从 1.4.3 升级到 1.4.4 (#10383)</span><span class="sxs-lookup"><span data-stu-id="d36bb-466">Bump PackageManagement version from 1.4.3 to 1.4.4 (#10383)</span></span>
- <span data-ttu-id="d36bb-467">更新 7.0.0-preview.4 的 README.md 和 metadata.json（内部 10011）</span><span class="sxs-lookup"><span data-stu-id="d36bb-467">Update README.md and metadata.json for 7.0.0-preview.4 (Internal 10011)</span></span>
- <span data-ttu-id="d36bb-468">将 .Net Core 3.0 版本从预览版 9 升级到 RC1 (#10552)（感谢 @bergmeister！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-468">Upgrade .Net Core 3.0 version from Preview 9 to RC1 (#10552) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="d36bb-469">修复 ExperimentalFeature 列表生成（内部 9996）</span><span class="sxs-lookup"><span data-stu-id="d36bb-469">Fix ExperimentalFeature list generation (Internal 9996)</span></span>
- <span data-ttu-id="d36bb-470">将 PSReadLine 版本从 2.0.0-beta4 升级到 2.0.0-beta5 (#10536)</span><span class="sxs-lookup"><span data-stu-id="d36bb-470">Bump PSReadLine version from 2.0.0-beta4 to 2.0.0-beta5 (#10536)</span></span>
- <span data-ttu-id="d36bb-471">修复发布生成脚本以设置发布标记</span><span class="sxs-lookup"><span data-stu-id="d36bb-471">Fix release build script to set release tag</span></span>
- <span data-ttu-id="d36bb-472">将 Microsoft.PowerShell.Native 的版本更新为 7.0.0-preview.2 (#10519)</span><span class="sxs-lookup"><span data-stu-id="d36bb-472">Update version of Microsoft.PowerShell.Native to 7.0.0-preview.2 (#10519)</span></span>
- <span data-ttu-id="d36bb-473">升级到 Netcoreapp3.0 preview9 (#10484)（感谢 @bergmeister！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-473">Upgrade to Netcoreapp3.0 preview9 (#10484) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="d36bb-474">确保每日协调生成，知道它是每日生成 (#10464)</span><span class="sxs-lookup"><span data-stu-id="d36bb-474">Make sure the daily coordinated build, knows it is a daily build (#10464)</span></span>
- <span data-ttu-id="d36bb-475">更新组合包生成以发布每日生成 (#10449)</span><span class="sxs-lookup"><span data-stu-id="d36bb-475">Update the combined package build to release the daily builds (#10449)</span></span>
- <span data-ttu-id="d36bb-476">删除 appveyor 引用 (#10445)（感谢 @RDIL！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-476">Remove appveyor reference (#10445) (Thanks @RDIL!)</span></span>
- <span data-ttu-id="d36bb-477">将 NJsonSchema 版本从 10.0.22 升级到 10.0.23 (#10421)</span><span class="sxs-lookup"><span data-stu-id="d36bb-477">Bump NJsonSchema version from 10.0.22 to 10.0.23 (#10421)</span></span>
- <span data-ttu-id="d36bb-478">取消删除 linux-x64 生成文件夹，因为 Alpine 的某些依赖项需要它 (#10407)</span><span class="sxs-lookup"><span data-stu-id="d36bb-478">Remove the deletion of linux-x64 build folder because some dependencies for Alpine need it (#10407)</span></span>

### <a name="documentation-and-help-content"></a><span data-ttu-id="d36bb-479">文档和帮助内容</span><span class="sxs-lookup"><span data-stu-id="d36bb-479">Documentation and Help Content</span></span>

- <span data-ttu-id="d36bb-480">将更改日志重构到每个版本的一个日志中 (#11165)</span><span class="sxs-lookup"><span data-stu-id="d36bb-480">Refactor change logs into one log per release (#11165)</span></span>
- <span data-ttu-id="d36bb-481">修复 PowerShell 7 联机帮助文档的 Fwlink (#11071)</span><span class="sxs-lookup"><span data-stu-id="d36bb-481">Fix FWLinks for PowerShell 7 online help documents (#11071)</span></span>
- <span data-ttu-id="d36bb-482">更新 CONTRIBUTING.md (#11096)（感谢 @mklement0！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-482">Update CONTRIBUTING.md (#11096) (Thanks @mklement0!)</span></span>
- <span data-ttu-id="d36bb-483">修复 README.md 中的安装文档链接 (#11083)</span><span class="sxs-lookup"><span data-stu-id="d36bb-483">Fix installation doc links in README.md (#11083)</span></span>
- <span data-ttu-id="d36bb-484">向 install-powershell.ps1 脚本添加示例 (#11024)（感谢 @kilasuit！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-484">Adds examples to install-powershell.ps1 script (#11024) (Thanks @kilasuit!)</span></span>
- <span data-ttu-id="d36bb-485">修复 CHANGELOG.md 中的 Select-String 强调和 Import-DscResource (#10890)</span><span class="sxs-lookup"><span data-stu-id="d36bb-485">Fix to Select-String emphasis and Import-DscResource in CHANGELOG.md (#10890)</span></span>
- <span data-ttu-id="d36bb-486">从 powershell-beginners-guide.md 中删除过期链接 (#10926)</span><span class="sxs-lookup"><span data-stu-id="d36bb-486">Remove the stale link from powershell-beginners-guide.md (#10926)</span></span>
- <span data-ttu-id="d36bb-487">合并稳定且提供服务的更改日志 (#10527)</span><span class="sxs-lookup"><span data-stu-id="d36bb-487">Merge stable and servicing change logs (#10527)</span></span>
- <span data-ttu-id="d36bb-488">更新生成文档中已使用的 .NET 版本 (#10775)（感谢 @Greg-Smulko！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-488">Update used .NET version in build docs (#10775) (Thanks @Greg-Smulko!)</span></span>
- <span data-ttu-id="d36bb-489">将 MSDN 中的链接替换为 powershell-beginners-guide.md 中的 docs.microsoft.com (#10778)（感谢 @iSazonov！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-489">Replace links from MSDN to docs.microsoft.com in powershell-beginners-guide.md (#10778) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="d36bb-490">修复中断的 DSC 概述链接 (#10702)</span><span class="sxs-lookup"><span data-stu-id="d36bb-490">Fix broken DSC overview link (#10702)</span></span>
- <span data-ttu-id="d36bb-491">更新 Support_Question.md，以链接到作为另一个社区资源的堆栈溢出 (#10638)（感谢 @mklement0！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-491">Update Support_Question.md to link to Stack Overflow as another community resource (#10638) (Thanks @mklement0!)</span></span>
- <span data-ttu-id="d36bb-492">将处理器体系结构添加到分发请求模板 (#10661)</span><span class="sxs-lookup"><span data-stu-id="d36bb-492">Add processor architecture to distribution request template (#10661)</span></span>
- <span data-ttu-id="d36bb-493">将新的 PowerShell MoL 书籍添加到学习 PowerShell Docs (#10602)</span><span class="sxs-lookup"><span data-stu-id="d36bb-493">Add new PowerShell MoL book to learning PowerShell docs (#10602)</span></span>
- <span data-ttu-id="d36bb-494">更新 v6.1.6 和 v6.2.3 版本的 README.md 和元数据 (#10523)</span><span class="sxs-lookup"><span data-stu-id="d36bb-494">Update README.md and metadata for v6.1.6 and v6.2.3 releases (#10523)</span></span>
- <span data-ttu-id="d36bb-495">修复 README.md 中的拼写错误 (#10465)（感谢 @vedhasp！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-495">Fix a typo in README.md (#10465) (Thanks @vedhasp!)</span></span>
- <span data-ttu-id="d36bb-496">将对 PSKoans 模块的引用添加到学习资源文档 (#10369)（感谢 @vexx32！）</span><span class="sxs-lookup"><span data-stu-id="d36bb-496">Add a reference to PSKoans module to Learning Resources documentation (#10369) (Thanks @vexx32!)</span></span>
- <span data-ttu-id="d36bb-497">更新 7.0.0-preview.3 的 README.md 和 metadata.json（内部 #10393）</span><span class="sxs-lookup"><span data-stu-id="d36bb-497">Update README.md and metadata.json for 7.0.0-preview.3 (#10393)</span></span>
