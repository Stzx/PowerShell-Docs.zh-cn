...
对于给定模块中的所有资源，使用 **Test-xDscResource** 和 **Test-xDscSchema** 在资源模块中创建“Tests”文件夹、创建 `ResourceDesignerTests.ps1` 文件并添加测试是很好的做法。 通过这种方法你可以快速验证给定模块的所有资源的架构，并在发布前执行完整性检查。 对于 xRemoteFile，`ResourceTests.ps1` 可能和以下情况一样简单：
