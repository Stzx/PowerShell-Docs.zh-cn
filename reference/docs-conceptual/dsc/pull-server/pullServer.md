...
本地配置管理器 (LCM) 可由请求服务解决方案集中管理。 使用此方法时，会向服务注册要管理的节点并在 LCM 设置中指定一个配置。 会将配置以及作为配置依赖项所需的全部 DSC 资源下载到计算机，LCM 将使用它们来管理配置。
...
可将请求服务配置为在 Windows Server 上运行。 请注意，Windows Server 中包含的拉取服务解决方案只能存储配置和模块以供下载，并将报表数据捕获到数据库中。 它不具备 Azure 中的服务所提供的许多功能，因此不适合用于评估服务的使用方式。
