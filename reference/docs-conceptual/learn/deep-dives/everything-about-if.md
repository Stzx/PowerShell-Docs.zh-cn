...
## <a name="conditional-execution"></a>有条件执行

你的脚本通常需要做出决策，并根据这些决策执行不同的逻辑。
这就是有条件执行的意思。 需要计算一个语句或值，然后根据该计算结果执行不同的代码段。 这正是 `if` 语句能够发挥作用的地方。
...
在某些语言中，可以在 `if` 语句后放置一行代码，并执行该语句。 在 PowerShell 中情况并非如此。 必须提供带大括号的完整 `scriptblock` 才能使其正常工作。

## <a name="comparison-operators"></a>比较运算符

`if` 语句最常见的用法是比较两个项。 PowerShell 具有特殊运算符，可用于不同比较方案。 当使用比较运算符时，会将左右两侧的值进行比较。

### <a name="-eq-for-equality"></a>-eq（相等）
...
这些是 `-eq` 的反向变体。 当列出其他运算符的变体时，我将把这些类型组合在一起。
...
我不知道为什么这些运算符要使用区分大小写和不区分大小写的选项。
...
必须指出该模式与整个字符串匹配。 如果需要匹配字符串中间的某些内容，则需要在字符串的两端放置 `*`。
...
- `-ilike` 不区分大小写通配符
- `-clike` 区分大小写通配符
- `-notlike` 不区分大小写通配符不匹配
- `-inotlike` 不区分大小写通配符不匹配
- `-cnotlike` 区分大小写通配符不匹配
...
默认情况下，正则表达式模式匹配字符串的任意位置。 因此，你可以指定希望匹配的子字符串，如下所示：
...
- `-imatch` 不区分大小写正则表达式
- `-cmatch` 区分大小写正则表达式
- `-notmatch` 不区分大小写正则表达式不匹配
- `-inotmatch` 不区分大小写正则表达式不匹配
- `-cnotmatch` 区分大小写正则表达式不匹配
...
如果使用的是类或通过管道接受各种对象，则可以使用此项。 可以将服务或服务名称作为输入。 然后，查看是否有服务并提取服务（如果只有名称）。
...
对单个值使用前面的运算符时，结果为 `$true` 或 `$false`。 处理集合时，处理方式略有不同。 集合中的每一项都将进行计算，运算符将返回计算结果为 `$true` 的每个值。
...
我需要指出的是，此处的详细信息隐藏了一个小陷阱。以这种方式使用 `-ne` 运算符时，很容易错误地向后查看逻辑。 如果集合中的任何项与你的值都不匹配，则对集合使用 `-ne` 将返回 `$true`。
...
这个花招看起来巧妙，但我们有运算符 `-contains` 和 `-in` 可以更高效地处理这种情况。 并且 `-notcontains` 会执行你期望的操作。
...
`-not` 运算符将表达式从 `$false` 翻转到 `$true` 或从 `$true` 翻转到 `$false`。 下面是一个示例，我们希望在 `Test-Path` 为 `$false` 时执行操作。

```powershell
if ( -not ( Test-Path -Path $path ) )
```

我们讨论的大多数运算符都有一种变体，你在其中无需使用 `-not` 运算符。 但它还是有用武之地。
...
你可能会发现其他语言（如 C#）的用户更多地使用 `!`。 我更喜欢把它打出来，因为我发现在快速查看脚本时很难看清。
...
在此示例中，左侧的 `$age` 必须为 13 或更大，右侧必须小于 55。 我添加了额外的括号以使其更清晰，但只要表达式简单，它们就是可选的。 下面是同一个示例不使用括号的情况。
...
`-or` 允许指定两个表达式，并在其中一个表达式为 `$true` 时返回 `$true`。

```powershell
if ( $age -le 13 -or $age -ge 55 )
```

与 `-and` 运算符一样，计算从左到右进行。 例外情况是如果第一部分为 `$true`，则整个语句为 `$true`，且其不会处理表达式的其余部分。

另外，请注意语法如何应用于这些运算符。 需要两个单独的表达式。 我发现用户尝试执行类似于此 `$value -eq 5 -or 6` 的操作，但没有意识到他们的错误。

### <a name="-xor-exclusive-or"></a>-xor 异或

这个运算符有点不寻常。 `-xor` 仅允许一个表达式的计算结果为 `$true`。 因此，如果两个项均为 `$false` 或者均为 `$true`，则整个表达式为 `$false`。 另一种了解此运算符的方法是，仅当表达式的结果不同时，表达式才为 `$true`。

很少有人会使用此逻辑运算符，我也举不出一个好的使用示例。

## <a name="bitwise-operators"></a>位运算符

位运算符会对值中的位执行计算，并生成一个新值作为结果。 讲授[位运算符][]超出了本文的范围，下面仅列出这些运算符。
...
执行 `Test-Path` 时返回 `$true` 或 `$false`。 这也适用于返回其他值的命令。
...
在 `if` 语句中，如果没有结果或 `$null` 值的计算结果为 `$false`。 专门检查 `$null` 时，最佳做法是将 `$null` 放在左侧。
...
幸亏 [Prasoon Karunan V][] 提醒我，我差点忘了添加这一内容。
...
了解 `$first` 赋值是否没有输出，`$second` 赋值是否有输出？ 当在 `if` 语句中完成赋值时，它的执行方式就像上述 `$second` 赋值一样。 下面一个关于其使用方式的简单示例：
...
在此示例中，我们将检查 `$path`，确保它是一个文件。 如果找到该文件，则将其移动。 如果未找到，我们会编写一个警告。 这种类型的分支逻辑非常常见。
...
我们不仅限于单个条件检查。 我们可以将 `if` 和 `else` 语句链接在一起，而不是使用 `elseif` 语句来嵌套它们。
...
执行按自上而下的顺序进行。 首先计算顶级 `if` 语句。 如果为 `$false`，则会向下移动到列表中的下一个 `elseif` 或 `else`。 最后一个 `else` 是在其他任何语句未返回 `$true` 时要执行的默认操作。

### <a name="switch"></a>开关

现在，我要提到 `switch` 语句。 它提供了一个替代语法，用于对值执行多个比较。 使用 `switch`，你可以指定表达式，并将结果与多个不同值进行比较。 如果其中一个值匹配，则执行匹配的代码块。 请看下面的示例：
...
有三个可能的值可与 `$itemType` 匹配。 在这种情况下，它与 `Role` 匹配。 我使用了一个简单的示例来向你介绍 `switch` 运算符。 我在另一篇文章[关于 switch 语句的各项须知内容][]中介绍了更多内容。

## <a name="pipeline"></a>管道

管道是 PowerShell 一项独特的重要功能。 未禁止或赋给变量的任何值都将放在管道中。 `if` 以一种不总是显而易见的方式为我们提供了一种利用管道的方法。
...
每个脚本块将结果、命令或值放入管道。 然后，将 if 语句的结果赋给 `$discount` 变量。 该示例可以在每个脚本块中直接将这些值赋给 `$discount` 变量。 我不能说我经常将此方法与 `if` 语句一起使用，但我确实有一个最近使用的例子。

### <a name="array-inline"></a>数组内联

我有一个名为 [Invoke-SnowSql][] 的函数，该函数使用几个命令行参数启动可执行文件。 下面是此函数中的一个片段，可在其中生成参数数组。
...
我在初始化数组时以内联方式计算其结果。 如果 `$Debug` 为 true，则这些值将位于 `$snowSqlParam` 中的正确位置。 `$Path` 变量情况相同。
...
它们不便阅读，这使你更容易出错。 我们可以对此进行一些操作。

### <a name="line-continuation"></a>续行符

PowerShell 中提供了一些运算符，可让你将命令换到下一行。 如果要将表达式分为多行，则逻辑运算符 `-and` 和 `-or` 是理想的运算符。
...
这样感觉比上一个示例简洁得多。 你还可以使用变量名称来解释你真正检查的内容。 这也是节省不必要注释的自文档化代码的示例。

### <a name="multiple-if-statements"></a>多个 if 语句

我们可以将此语句分为多个语句，并一次查看一个语句。 在这种情况下，我们使用标志或跟踪变量来合并结果。
...
我不得不对逻辑进行反转操作，使标志逻辑正常工作。 每个计算都是单个 `if` 语句。 这样做的优点是，当你进行调试时，你可以确切地判断逻辑的作用。 我能够同时添加更好的详细程度。
...
你仍必须创建函数来执行验证，但它使得此代码更容易使用。 这使代码更易于测试。 在测试中，可以模拟对 `Test-ADDriveConfiguration` 的调用，而只需对此函数进行两次测试。 其中一个返回 `$true`，另一个返回 `$false`。 测试其他函数的方法更简单，因为它很小。

该函数的主体仍然可以是我们开始时使用的单行命令或我们在上一部分中使用的分离逻辑。 这适用于这两种方案，并使你能够在以后轻松地更改该实现。
...
如果你想要更深入地了解实际异常处理，请参阅我写的另一篇文章[关于异常的各项须知内容][]。

## <a name="final-words"></a>结束语

`if` 语句非常简单，但却是 PowerShell 的一个基本部分。 你会发现自己在编写的几乎每个脚本中都会多次使用此语句。 希望你比以往更好地理解这些内容。
