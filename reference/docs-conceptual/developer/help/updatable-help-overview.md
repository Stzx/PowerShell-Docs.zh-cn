...
- **版本支持**。 可更新帮助使用标准的四位位置（N1。N2.N3.N4）版本号。 当用户计算机（或目录）中的帮助文件的版本号低于 Internet 位置的帮助文件的版本号时，可更新帮助下载帮助文件 `Save-Help` 。

- **多语言支持**。 可更新的帮助支持多个 UI 区域性中的模块帮助文件。 可更新的帮助文件名包含标准语言代码（如 "en-us" 和 "ja-jp"）， `Update-Help` 并且和 `Save-Help` cmdlet 将帮助文件放在模块目录的特定于语言的子目录中。
