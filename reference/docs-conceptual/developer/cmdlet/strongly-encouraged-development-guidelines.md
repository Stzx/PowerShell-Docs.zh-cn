...
- [为 Cmdlet 名称使用特定名词 (SD01) ](./strongly-encouraged-development-guidelines.md#use-a-specific-noun-for-a-cmdlet-name-sd01)
...
- [ (SD05 创建 Cmdlet 帮助文件) ](./strongly-encouraged-development-guidelines.md#create-a-cmdlet-help-file-sd05)
...
- [使 Cmdlet 不区分大小写，不区分大小写 (SC04) ](./strongly-encouraged-development-guidelines.md#make-cmdlets-case-insensitive-and-case-preserving-sc04)
...
- 定义枚举类型 (或使用现有枚举类型) 指定有效值。 然后，使用枚举类型创建该类型的参数。
...
如果参数仅使用 `true` 和 `false` ，请将参数定义为类型[SwitchParameter](/dotnet/api/System.Management.Automation.SwitchParameter)。 开关参数 `true` 在命令中指定时被视为。 如果命令中未包含该参数，则 Windows PowerShell 会将参数的值视为 `false` 。 不要定义布尔参数。
...
例如，你可以将 `Mode` 脚本属性添加到[FileInfo](/dotnet/api/System.IO.FileInfo)类型，以更清晰地显示文件的属性。 此外，您还可以向 system.string `Count` 类型添加 alias [System.Array](/dotnet/api/System.Array)属性，以允许 (的属性名称的使用一致，而不是 `Length`) 。
...
### <a name="make-cmdlets-case-insensitive-and-case-preserving-sc04"></a>使 Cmdlet 不区分大小写，不区分大小写 (SC04) 
