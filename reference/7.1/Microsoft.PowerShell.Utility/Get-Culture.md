...
`Get-Culture`Cmdlet 获取有关当前区域性设置的信息。
这包括有关系统上的当前语言设置的信息（例如键盘布局）以及项的显示格式（例如数字、货币和日期）。

你还可以使用 `Get-UICulture` cmdlet 来获取系统上的当前用户界面区域性，并使用国际模块中的 [集区域性](/powershell/module/international/set-culture?view=win10-ps) cmdlet。
...
第二个命令显示区域性对象的所有属性。
它使用管道运算符 (|) 将区域性对象发送 `$C` 到 `Format-List` cmdlet。
